...
Package object references use a signed index convention in UE3 and UE4:

```text
0       = null/root special case depending field
> 0     = ExportMap[index - 1]
< 0     = ImportMap[-index - 1]
```

UE3 defines `PACKAGE_INDEX` as `INT`; UE4 wraps the same mapping in `FPackageIndex`.

Earlier compact-index generations preserve the same semantic import/export sign convention where their linker source uses package indices, but the binary integer encoding may be compact.

## Validation

For an import reference `p < 0`, compute `i = -p - 1` using overflow-safe arithmetic and require `0 <= i < ImportCount` before access.

Zero is not import index zero.

## Conformance

Keep raw package index and resolved map/index separately. Never convert all references to zero-based indices without retaining whether the reference was import, export or null.
