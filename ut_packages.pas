...
Contributor(s):          
  Neil Popplewell aka Neo4E656F/ttl kindly shared information about UT2003 SkeletalMesh/Animation classes.
...
type
  ...
  public
    ...
    procedure skipBytes (stream: tstream;n:integer);
    procedure skipOver (stream: tstream;const bytes:string);
    procedure skipTo (stream: tstream;const bytes:string);
...
begin
  ...
  else if (ext = '.pkg') or (ext = '.umj') or (ext = '.tvm') or (ext = '.hke') or (ext = '.mopp') then
    FGameHint := UTPGH_TribesVengeance
  ;
end;
      ...
      if (FLicenseeMode>=25) and (FLicenseeMode<=29) then
        begin
          if FVersion<122 then
            FGameHint:=UTPGH_UnrealTournament2003
          else if (FLicenseeMode=27) and (FVersion=130) then
            FGameHint:=UTPGH_TribesVengeance
          else
...
procedure TUTPackage.skipBytes(stream: tstream; n: integer);
begin
  stream.Seek(n,soFromCurrent);
end;

procedure TUTPackage.skipOver(stream: tstream; const bytes: string);
var tmp:string;b:integer;
begin
{
  setlength(tmp,length(bytes));
  repeat
    b:=stream.Read(tmp[1],length(bytes));
    if (tmp=bytes) and (b=length(bytes)) then
      break
    else
      stream.Seek(-length(bytes)+1,soFromCurrent);
  until b=0;
}
  setlength(tmp,length(bytes));
  b:=stream.Read(tmp[1],length(bytes)-1);
  if b=length(bytes)-1 then
    begin
      repeat
        b:=stream.Read(tmp[length(bytes)],1);
        if (tmp=bytes) and (b=1) then
          break
        else
          tmp:=copy(tmp,2,maxint)+#0;
      until b=0;
    end;
end;

procedure TUTPackage.skipTo(stream: tstream; const bytes: string);
var tmp:string;b:integer;
begin
{
  setlength(tmp,length(bytes));
  repeat
    ...
    if (tmp=bytes) and (b=length(bytes)) then
      begin
        stream.Seek(-length(bytes),soFromCurrent);
        break;
      end
    else
      stream.Seek(-length(bytes)+1,soFromCurrent);
...
var
  node,n,a: integer;
begin
  if (FOwner.GameHint=UTPGH_TribesVengeance) and (FOwner.FVersion>=130) then
    begin
      FOwner.read_dword(buffer,utdmUnknown); // 3
      FOwner.read_dword(buffer,utdmUnknown); // 0
      FOwner.read_dword(buffer,utdmUnknown); // 1
    end;
...
function TUTBitmap.AsBitmapBest: TBitmap;
               ...
               for y:=0 to FHeight-1 do
                 for x:=0 to FWidth-1 do
                   begin
                     ...
                     if (c<>0) then v:=v or integer($FF000000) else v:=v and $00FFFFFF;
              ...
              for y := 0 to yd4 - 1 do
                for x := 0 to xd4 - 1 do
                  begin
                    ...
                    else if format in [TEXF_DXT5] then
                       begin
                        ...
                        for bx := 0 to 3 do
                          begin
                            if (y * 4 + by < MipMap.height) and (x * 4 + bx < MipMap.width) then
                              ...
                              else if format in [TEXF_DXT3] then
                                begin
    ...
    begin
      if (FOwner.GameHint=UTPGH_TribesVengeance) and (FOwner.FVersion>=130) then
        begin
          ...
          FOwner.read_dword(buffer,utdmUnknown); // 0
          FOwner.read_dword(buffer,utdmUnknown); // 1
          FOwner.read_dword(buffer,utdmUnknown); // position after next array
          FOwner.read_idx(buffer,utdmUnknown); // unknown array count
          
        end;
      if FOwner.Version >= 63 then
        begin
          if (FOwner.FGameHint=UTPGH_MenOfValor) then
            FOwner.read_dword(buffer)
          else
          ...
          if (FOwner.FGameHint=UTPGH_Lineage2) or
             (FOwner.FGameHint=UTPGH_MenOfValor) then
...
begin
  ...
  if (ClassType=TUTObjectClassSkeletalMesh) and (FOwner.FVersion>=100) then exit;
...
begin
  ...
  if (FOwner.gamehint=UTPGH_TribesVengeance) and (FOwner.Version>=130) then
    begin
      FOwner.read_dword(buffer);
      FOwner.read_idx(buffer);
    end;
  ...
  if ((FOwner.Version>=120) or (FOwner.GameHint=UTPGH_Unreal2)) and (FOwner.GameHint<>UTPGH_Lineage2) then
    begin
      ...
      if (FOwner.GameHint=UTPGH_TribesVengeance) and (FOwner.FVersion>=130) then
        begin
          FOwner.read_byte(buffer);
          FOwner.read_idx(buffer,utdmRefToObject,'ProcessedText');
        end;
    end;
...
begin
  ...
  if FOwner.Version>=100{117} then
    begin
      ...
      if (FOwner.GameHint=UTPGH_TribesVengeance) and (FOwner.FVersion>=130) then
        begin
          FOwner.read_guid(buffer);
          FOwner.read_sizedasciiz(buffer); // properties in text form
          FOwner.read_sizedasciiz(buffer); // source filename
          FOwner.read_sizedasciiz(buffer); // always empty?
          FOwner.read_sizedasciiz(buffer); // always empty?
          FOwner.read_sizedasciiz(buffer); // always empty?
        end;
    end;
...
var
  size, a, b, vertexCount, triangleCount, lodcount, l: integer;x:string;v:TUT_Struct_Vector;
  w:word;f:single;
  MaterialInfo:array of record MaterialIndex,FirstTriangleIndex,TriangleCount:integer;end;
begin
  if FOwner.Version=80 then
    ...
    begin
      // This was possible thanks to N4UExtractor sources by xTTL
      // Some of the meshes from UT2003/2004 can be read. Specifically the player models and some others.
      inherited;
      FOwner.read_dword(buffer);
      vertexCount:=FOwner.read_word(buffer,utdmAsValue,'VertexCount');
      FOwner.read_word(buffer);
      FOwner.read_idx(buffer);
      size:=FOwner.read_idx(buffer,utdmAsValue,'TextureCount');
      setlength(FMaterials,CheckArrayLength(size));
      setlength(FTextures,CheckArrayLength(size));
      for a:=0 to size-1 do
        begin
          FTextures[a].Value:=FOwner.read_idx(buffer,utdmRefToObject,'TextureIndex');
          FMaterials[a].Flags:=0;
          FMaterials[a].TextureIndex:=a;
        end;

      FOwner.read_float(buffer);
      FOwner.read_float(buffer);
      FOwner.read_float(buffer);
      FOwner.skipBytes(buffer,24); // dword(0), dword(0), float, dword(0), dword, dword(0)

      triangleCount:=FOwner.read_idx(buffer,utdmAsValue,'TriangleCount');
      ...
      //--
      FOwner.read_word(buffer); // 00
      size:=FOwner.read_idx(buffer);
      FOwner.read_dword(buffer); // 0
      for a:=1 to size+2 do
        begin
          FOwner.read_dword(buffer);
          FOwner.read_float(buffer);
        end;
      //--FOwner.skipOver(buffer,#$9A#$99#$99#$3E);

      FOwner.skipBytes(buffer,64); // 00
      FOwner.read_word(buffer);
      repeat
        size:=FOwner.read_idx(buffer,utdmAsValue,'BoneCount');
      until size<>0; // failsafe
      for a:=0 to size-1 do
        Read_Struct_MeshBone (FOwner,buffer,'RefSkeleton');

      // TODO : meshes that don't work change the format here

      for a:=0 to 9 do FOwner.read_byte(buffer); // 01 0B 00 00 00 00 00 00 00 00
      lodcount:=FOwner.read_idx(buffer);
      for l:=0 to lodcount-1 do
        begin
          (*
          repeat
            w1:=FOwner.read_word(buffer);
            w2:=FOwner.read_word(buffer);
            if (w1<>$FFFF) and (w2<>$FFFF) then
              begin
                FOwner.read_float(buffer);
                FOwner.read_float(buffer);
              end;
          until (w1=$FFFF) and (w2=$FFFF);
          *)
          FOwner.skipOver(buffer,#$FF#$FF#$FF#$FF); // since i don't understand yet the end of the loop, i use this hack

          size:=FOwner.read_idx(buffer,utdmAsValue,'VertexCount');
          if l=0 then setlength(FPoints,CheckArrayLength(size));
          for a:=0 to size-1 do
            begin
              v := Read_Struct_Vector(FOwner, buffer,'Points');
              if l=0 then FPoints[a] := v;
              FOwner.read_word (buffer);
              FOwner.read_word (buffer);
            end;
          FOwner.read_dword(buffer,utdmAsValue,'RealVertexCount');
          size:=FOwner.read_idx(buffer,utdmAsValue,'MaterialInfoCount');
          if l=0 then setlength(MaterialInfo,size);
          for a:=0 to size-1 do
            begin
              w:=FOwner.read_word(buffer,utdmAsValue,'MaterialIndex');
              if l=0 then MaterialInfo[a].MaterialIndex:=w;
              FOwner.read_word(buffer);
              FOwner.read_word(buffer,utdmAsValue,'FirstWedgeIndex');
              FOwner.read_word(buffer,utdmAsValue,'LastWedgeIndex');
              FOwner.read_dword(buffer,utdmAsValue,'Flags');
              FOwner.read_word(buffer);
              w:=FOwner.read_word(buffer,utdmAsValue,'FirstTriangleIndex');
              if l=0 then MaterialInfo[a].FirstTriangleIndex:=w;
              w:=FOwner.read_word(buffer,utdmAsValue,'TriangleCount');
              if l=0 then MaterialInfo[a].TriangleCount:=w;
            end;
          FOwner.read_byte(buffer);
          size:=FOwner.read_idx(buffer); // 3 * TriangleCount
          if l=0 then setlength(FFaces,CheckArrayLength(triangleCount));
          for a:=0 to (size div 3)-1 do
            begin
              w:=FOwner.read_word(buffer,utdmAsValue,'WedgeIndex1');
              if l=0 then FFaces[a].WedgeIndex1:=w;
              w:=FOwner.read_word(buffer,utdmAsValue,'WedgeIndex2');
              if l=0 then FFaces[a].WedgeIndex2:=w;
              w:=FOwner.read_word(buffer,utdmAsValue,'WedgeIndex3');
              if l=0 then FFaces[a].WedgeIndex3:=w;
              if l=0 then
                begin
                  FFaces[a].MatIndex:=-1;
                  for b:=0 to high(MaterialInfo) do
                    if (MaterialInfo[b].FirstTriangleIndex<=a) and
                       (a<MaterialInfo[b].FirstTriangleIndex+MaterialInfo[b].TriangleCount) then
                      begin
                        FFaces[a].MatIndex:=MaterialInfo[b].MaterialIndex;
                        break;
                      end;
                end;
            end;

          FOwner.skipBytes(buffer,22);
          // examples from same object: 
          // 22 bytes = 01 00 00 00 00 00 00 00 00 01 00 00 00 00 00 00 00 00 00 00 00 00
          // 22 bytes = 00 00 00 00 00 00 00 00 00 01 00 00 00 00 00 00 00 00 00 00 00 00
          // 22 bytes = 01 00 00 00 00 00 00 00 00 01 00 00 00 00 00 00 00 00 00 00 00 00
          // 22 bytes = 00 00 00 00 00 00 00 00 00 01 00 00 00 00 00 00 00 00 00 00 00 00

          FOwner.read_dword(buffer); // skipoverarray
          size:=FOwner.read_idx(buffer,utdmAsValue,'BoneInfluenceCount');
          for a := 0 to size - 1 do
            begin
              FOwner.read_float(buffer,'Weight');
              FOwner.read_word(buffer,utdmAsValue,'PointIndex');
              FOwner.read_word(buffer,utdmAsValue,'BoneIndex');
            end;
          FOwner.read_dword(buffer); // skipoverarray
          size:=FOwner.read_idx(buffer,utdmAsValue,'WedgeCount');
          if l=0 then setlength(FWedges,CheckArrayLength(size));
          for a:=0 to size-1 do
            begin
              w:=FOwner.read_word(buffer,utdmAsValue,'VertexIndex');
              if l=0 then FWedges[a].VertexIndex:=w;
              f:=FOwner.read_float(buffer,'U');
              if l=0 then FWedges[a].U:=trunc(255*f);
              f:=FOwner.read_float(buffer,'V');
              if l=0 then FWedges[a].V:=trunc(255*f);
            end;

          FOwner.read_dword(buffer); // to jump over array
          size:=FOwner.read_idx(buffer,utdmAsValue,'Unknown Count'); // triangles (vertex indexes + material) ?
          for a:=0 to size-1 do
            begin
              FOwner.read_word(buffer);
              FOwner.read_word(buffer);
          ...
          FOwner.read_dword(buffer); // to jump over array
          size:=FOwner.read_idx(buffer,utdmAsValue,'Unknown Count');
          for a:=0 to size-1 do
            begin
              FOwner.read_float(buffer);
              FOwner.read_float(buffer);
          ...
          FOwner.read_float(buffer); // 1.0, 0.50, 0.28, 0.17
          FOwner.read_float(buffer); // 0.02

          exit; // we don't need the rest, so we stop here to avoid problems

          // the following block is still completely unknown so we can't read it
          // examples from the same file:
          // 18 bytes: 45 02 00 00 04 00 00 00 00 00 00 00 00 00 00 00 60 47
          // 19 bytes: 04 02 00 00 01 00 00 00 00 00 00 00 00 00 00 00 7F 28 60
          // 18 bytes: 46 01 00 00 01 00 00 00 00 00 00 00 00 00 00 00 59 0C
          // 17 bytes: 62 00 00 00 01 00 00 00 00 00 00 00 00 00 00 00 00
        end;

        FOwner.skipTo(buffer,FOwner.EncodeIndex(vertexcount)); // hack to find the array count
        //FOwner.read_dword(buffer); // to jump over array
        size:=FOwner.read_idx(buffer,utdmAsValue,'Unknown Count');
        for a:=0 to size-1 do
          begin
...
(*
    begin
      //raise EUnknownObjectFormat.create ('The new SkeletalMesh format is unknown.');
      inherited;
      FOwner.read_dword(buffer);
      FOwner.read_word(buffer,utdmAsValue,'VertexCount');
      FOwner.read_word(buffer);
      FOwner.read_idx(buffer);
      size:=FOwner.read_idx(buffer,utdmAsValue,'TextureCount');
      setlength(FMaterials,CheckArrayLength(size));
      ...
      //--
      FOwner.skipTo(buffer,#00#00#01#00#02#00);
      FOwner.skipBytes(buffer,-2); // we suppose a 2 byte index

      // the following seems to repeat:
      ...
      for a:=0 to size-1 do
        FOwner.read_word(buffer);
      FOwner.read_dword(buffer); // 5
      FOwner.read_dword(buffer); // 6
      FOwner.read_dword(buffer); // 0
      FOwner.read_dword(buffer); // 0
      size:=FOwner.read_idx(buffer,utdmAsValue,'Unknown Count');
      for a:=0 to size-1 do
      ...
      FOwner.read_dword(buffer); // to jump over array
      size:=FOwner.read_idx(buffer,utdmAsValue,'Unknown Count'); // boneinfluences
      for a:=0 to size-1 do
        begin
          FOwner.read_float(buffer);
          FOwner.read_word(buffer);
      ...
      FOwner.read_dword(buffer); // to jump over array
      size:=FOwner.read_idx(buffer,utdmAsValue,'Unknown Count'); // wedges U V
      for a:=0 to size-1 do
        begin
          FOwner.read_word(buffer);
          FOwner.read_float(buffer);
      ...
      x:={#0+}FOwner.EncodeIndex(size);
      FOwner.skipTo(buffer,x); // we jump to the next array that has the same count than the previous one
      // we also skip any other iteration

      {FOwner.read_dword(buffer);    // these next blocks do not have the correct types
      FOwner.read_dword(buffer);
      FOwner.read_dword(buffer);
      FOwner.read_dword(buffer);
      FOwner.read_dword(buffer);
      ...
      FOwner.read_dword(buffer); // to jump over array
      size:=FOwner.read_idx(buffer,utdmAsValue,'Unknown Count'); // bone influences
      for a:=0 to size-1 do
        begin
          FOwner.read_float(buffer);
          FOwner.read_word(buffer);
      ...
      FOwner.read_dword(buffer); // to jump over array
      size:=FOwner.read_idx(buffer,utdmAsValue,'Unknown Count'); // wedge U V
      for a:=0 to size-1 do
        begin
          FOwner.read_word(buffer);
          FOwner.read_float(buffer);
      ...
      for a:=0 to size-1 do
        begin
          ...
          FOwner.read_word(buffer);
          FOwner.read_word(buffer);
          FOwner.read_dword(buffer);  // flags?
          FOwner.read_word(buffer);
        end;
      FOwner.read_dword(buffer); // to jump over array
      size:=FOwner.read_idx(buffer,utdmAsValue,'Unknown Count');
      ...
      FOwner.read_dword(buffer);
      FOwner.read_dword(buffer); // 00
      FOwner.read_byte(buffer); // 00
    end;*)
end;
