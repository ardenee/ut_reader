...
final class UE4BinaryReader
{
    ...
    public function fstring(): string
    {
        ...
        if ($length === 0) return '';
        if ($length > 0) {
            if ($length > 1048576 || $length > $this->remaining()) throw new OutOfBoundsException("bad FString length=$length pos={$this->pos}");
        ...
        $chars = -$length;
        $bytes = $chars * 2;
        if ($chars > 524288 || $bytes > $this->remaining()) throw new OutOfBoundsException("bad wide FString length=$length pos={$this->pos}");
    ...
    {
        $a = $this->u32(); $b = $this->u32(); $c = $this->u32(); $d = $this->u32();
...
final class UnrealPackageReader4
{
    ...
    {
        return ['signature'=>0,'legacyFileVersion'=>0,'legacyUE3Version'=>0,'version'=>0,'licenseeVersion'=>0,'unversioned'=>false,'customVersions'=>[],
            'totalHeaderSize'=>0,'folderName'=>'','packageFlags'=>0,'nameCount'=>0,'nameOffset'=>0,'gatherableTextDataCount'=>0,'gatherableTextDataOffset'=>0,
    ...
    private function parse(): void
    {
        ...
        $tag = $r->u32();
        if ($tag !== self::PACKAGE_FILE_TAG && $tag !== self::PACKAGE_FILE_TAG_SWAPPED) throw new RuntimeException(sprintf('Bad UE4 package tag 0x%08X', $tag));
        if ($tag === self::PACKAGE_FILE_TAG_SWAPPED) throw new RuntimeException('Big-endian swapped UE4 packages are not supported yet.');
        ...
        if ($ue4Version >= self::VER_ADDED_SEARCHABLE_NAMES) $this->header['searchableNamesOffset'] = $r->i32();
        ...
        if ($legacy > -7) $this->header['textureAllocationsRemoved'] = $r->i32();
        $this->header['assetRegistryDataOffset'] = $r->i32();
        $this->header['bulkDataStartOffset'] = $r->i64();
        if ($ue4Version >= self::VER_WORLD_LEVEL_INFO) $this->header['worldTileInfoDataOffset'] = $r->i32();
        if ($ue4Version >= self::VER_CHANGED_CHUNKID_TO_BE_AN_ARRAY_OF_CHUNKIDS) $this->header['chunkIDs'] = $this->readIntArray($r);
        elseif ($ue4Version >= self::VER_ADDED_CHUNKID_TO_ASSETDATA_AND_UPACKAGE) { $chunkId = $r->i32(); if ($chunkId >= 0) $this->header['chunkIDs'][] = $chunkId; }
    ...
    {
        $count = $r->i32();
        if ($count < 0 || $count > 65536) throw new RuntimeException('Bad compressed chunk count ' . $count . ' at ' . ($r->tell() - 4));
        $out = [];
    ...
    {
        $count = $r->i32();
        if ($count < 0 || $count > 65536) throw new RuntimeException('Bad string array count ' . $count . ' at ' . ($r->tell() - 4));
        $out = [];
    ...
    {
        $count = $r->i32();
        if ($count < 0 || $count > 1048576) throw new RuntimeException('Bad int array count ' . $count . ' at ' . ($r->tell() - 4));
        $out = [];
    ...
    private function validateTableBounds(): void
    {
        foreach ([['name', 'nameCount', 'nameOffset'], ['import', 'importCount', 'importOffset'], ['export', 'exportCount', 'exportOffset']] as $t) {
            [$label, $countKey, $offsetKey] = $t;
            $count = (int)($this->header[$countKey] ?? 0);
            $offset = (int)($this->header[$offsetKey] ?? 0);
            if ($count < 0) $this->issues[] = "Bad $label count: $count";
