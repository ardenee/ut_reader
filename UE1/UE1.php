...
function jumpToRef(targetId){if(!targetId)return;if(targetId.indexOf('ref-import-')===0){showPanel('tables-panel');showSub('imports-sub');}else if(targetId.indexOf('ref-export-')===0){showPanel('tables-panel');showSub('exports-sub');}window.setTimeout(()=>{const target=document.getElementById(targetId);if(!target)return;document.querySelectorAll('.tree-node.is-target').forEach(e=>e.classList.remove('is-target'));openContainingDetails(target);target.classList.add('is-target');target.scrollIntoView({behavior:'smooth',block:'center'});},40);}
document.addEventListener('click',function(ev){const a=ev.target.closest&&ev.target.closest('a.ref-link');if(!a)return;const href=a.getAttribute('href')||'';if(href.charAt(0)!=='#')return;ev.preventDefault();history.replaceState(null,'',href);jumpToRef(href.substring(1));});
window.addEventListener('load',function(){if(location.hash){jumpToRef(location.hash.substring(1));}});
</script>
</head>
<body>
<div class="workspace">
