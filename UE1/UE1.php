...
function renderImportNode(UnrealPackageReader $pkg, array $imports, array $importsByOuter, int $idx, bool $includeChildren, string $idPrefix = ''): void
{
    ...
    <details id="<?= h($anchor) ?>" class="tree-node" data-filter-row>
        ...
        <div class="tree-lines">
            <div>Object:<span class="mono"><?= h($object) ?></span><a class="ref-tag ref-link" href="#<?= h(object_ref_target_id($ref)) ?>" data-ref="<?= h($ref) ?>">(<?= h($ref) ?>)</a></div>
...
function clearTargetState(){document.querySelectorAll('.tree-node.is-target').forEach(e=>e.classList.remove('is-target'));}
function jumpToRef(targetId){if(!targetId)return;if(targetId.indexOf('ref-import-')===0){showPanel('tables-panel');showSub('imports-sub');}else if(targetId.indexOf('ref-export-')===0){showPanel('tables-panel');showSub('exports-sub');}window.setTimeout(()=>{const target=document.getElementById(targetId);if(!target)return;clearTargetState();openContainingDetails(target);target.classList.add('is-target');target.scrollIntoView({behavior:'smooth',block:'center'});},60);}
document.addEventListener('click',function(ev){const a=ev.target.closest&&ev.target.closest('a.ref-link');if(!a)return;const href=a.getAttribute('href')||'';if(href.charAt(0)!=='#')return;ev.preventDefault();history.replaceState(null,'',window.location.pathname+window.location.search);jumpToRef(href.substring(1));});
