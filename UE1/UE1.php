...
{
    [$name, $ref] = split_ref_for_display($label);
    echo '<span class="mono">' . h($name) . '</span>';
    if ($ref !== '') {
        echo '<span class="ref-tag">' . h($ref) . '</span>';
    }
}
...
function renderImportNode(UnrealPackageReader $pkg, array $imports, array $importsByOuter, int $idx, bool $includeChildren): void
{
    ...
    <details class="tree-node" data-filter-row>
        ...
        <div class="tree-lines">
            <div>Object:<span class="mono"><?= h($object) ?></span><span class="ref-tag">(<?= h($ref) ?>)</span></div>
