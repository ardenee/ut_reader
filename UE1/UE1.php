...
function flags_text(array $flags): string
{
    return $flags ? ' (' . implode(', ', $flags) . ')' : '';
}

function ref_text(UnrealPackageReader $pkg, int $ref): string
{
    if ($ref === 0) {
        return '';
    }
    ...
    if ($ref > 0) {
        return $pkg->exportObjectName($ref - 1);
    }

    return $pkg->importObjectName((-1 * $ref) - 1);
}

$importsByOuter = [];
foreach ($imports as $idx => $im) {
    $outer = (int)($im['outerIndex'] ?? 0);
    $importsByOuter[$outer][] = $idx;
}

$exportsByOuter = [];
foreach ($exports as $idx => $ex) {
    $outer = (int)($ex['packageIndex'] ?? $ex['outerIndex'] ?? 0);
    $exportsByOuter[$outer][] = $idx;
}
...
$renderImportNode = function (int $idx, int $depth = 0) use (&$renderImportNode, &$importsByOuter, $imports, $pkg): void {
    ...
    <details class="tree-node" open>
        <summary><span class="tree-icon">▣</span><span class="tree-title"><?= h($object) ?></span><span class="tree-ref">(<?= h($ref) ?>)</span></summary>
        <div class="tree-lines">
            <div>Object: <span class="mono"><?= h($object) ?></span><?= h('(' . $ref . ')') ?></div>
            <div>Class: <span class="mono"><?= h($class) ?></span></div>
            <div>Package: <span class="mono"><?= h($package) ?></span></div>
            <?php if ($outer !== ''): ?><div>Outer: <span class="mono"><?= h($outer) ?></span></div><?php endif; ?>
            <?php foreach ($children as $childIdx): ?>
                <?php $renderImportNode((int)$childIdx, $depth + 1); ?>
            <?php endforeach; ?>
        </div>
    </details>
    <?php
...
$renderExportNode = function (int $idx, int $depth = 0) use (&$renderExportNode, &$exportsByOuter, $exports, $pkg): void {
    $ex = $exports[$idx] ?? null;
    if (!is_array($ex)) {
        return;
    }
    ...
    ?>
    <details class="tree-node content-node" open>
        <summary><span class="tree-icon">≡</span><span class="tree-title"><?= h($object) ?></span><span class="tree-class"><?= h($class) ?></span></summary>
        <div class="tree-lines">
            <div>ObjectFlags: <span class="mono"><?= h(sprintf('%08X', (int)($ex['objectFlags'] ?? 0))) ?></span></div>
            <div>Object: <span class="mono"><?= h($object) ?></span><?= h('(' . $ref . ')') ?></div>
            ...
            <?php foreach ($children as $childIdx): ?>
                <?php $renderExportNode((int)$childIdx, $depth + 1); ?>
            <?php endforeach; ?>
        </div>
    </details>
    <?php
...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    .toolbtn { border: 1px solid var(--b); background: white; border-radius: 4px; padding: 5px 10px; cursor: default; }
    .searchbox { width: 100%; max-width: 360px; padding: 6px 8px; border: 1px solid #9aa7b1; font-size: 14px; }
    .package-name { color: var(--sub); }

    .main-tabs { display: flex; gap: 0; padding: 0 12px; border-bottom: 1px solid var(--b); background: #f8fafb; }
    .main-tab { border: 0; border-right: 1px solid var(--b); background: transparent; padding: 9px 14px; font-weight: 600; color: #344054; cursor: pointer; }
    .main-tab:first-child { border-left: 1px solid var(--b); }
    .main-tab.active { background: white; color: var(--accent); box-shadow: inset 0 -2px 0 var(--accent); }
    ...
    .flag-table { margin-top: 24px; width: 100%; max-width: 780px; border-collapse: collapse; }
    .flag-table th, .flag-table td { border: 1px solid var(--b); padding: 6px 8px; }
    .flag-table th { text-align: left; background: var(--muted); }
    .flag-true { background: #0078d7; color: white; }

    .subtabs { display: flex; gap: 0; margin-bottom: 10px; border-bottom: 1px solid var(--b); }
    .subtab { border: 0; background: #f8fafb; padding: 7px 12px; border-right: 1px solid var(--b); cursor: pointer; font-weight: 600; }
    .subtab.active { background: white; color: var(--accent); box-shadow: inset 0 -2px 0 var(--accent); }
    .subpanel { display: none; }
    .subpanel.active { display: block; }

    table.data { border-collapse: collapse; width: 100%; table-layout: fixed; margin: 8px 0 18px; }
    table.data th, table.data td { border: 1px solid var(--b); padding: 6px 8px; vertical-align: top; }
    table.data th { background: var(--muted); text-align: left; }
    table.data td { background: #fff; }
    .names-table th:nth-child(1), .names-table td:nth-child(1) { width: 22%; }
    .names-table th:nth-child(2), .names-table td:nth-child(2) { width: 46%; }
    .names-table th:nth-child(3), .names-table td:nth-child(3) { width: 10%; }
    .names-table th:nth-child(4), .names-table td:nth-child(4) { width: 22%; }
    .imports-table th:nth-child(1), .imports-table td:nth-child(1) { width: 15%; }
    .imports-table th:nth-child(2), .imports-table td:nth-child(2) { width: 15%; }
    .imports-table th:nth-child(3), .imports-table td:nth-child(3) { width: 18%; }
    .imports-table th:nth-child(4), .imports-table td:nth-child(4) { width: 18%; }
    .imports-table th:nth-child(5), .imports-table td:nth-child(5) { width: 9%; }
    .imports-table th:nth-child(6), .imports-table td:nth-child(6) { width: 25%; }
    .exports-table th:nth-child(1), .exports-table td:nth-child(1) { width: 9%; }
    .exports-table th:nth-child(2), .exports-table td:nth-child(2) { width: 8%; }
    .exports-table th:nth-child(3), .exports-table td:nth-child(3) { width: 8%; }
    .exports-table th:nth-child(4), .exports-table td:nth-child(4) { width: 11%; }
    .exports-table th:nth-child(5), .exports-table td:nth-child(5) { width: 20%; }
    .exports-table th:nth-child(6), .exports-table td:nth-child(6) { width: 7%; }
    .exports-table th:nth-child(7), .exports-table td:nth-child(7) { width: 8%; }
    .exports-table th:nth-child(8), .exports-table td:nth-child(8) { width: 7%; }
    .exports-table th:nth-child(9), .exports-table td:nth-child(9) { width: 8%; }
    .exports-table th:nth-child(10), .exports-table td:nth-child(10) { width: 14%; }

    .tree { font-family: Segoe UI, Tahoma, Arial, sans-serif; max-width: 900px; }
...
function showPanel(id) {
    document.querySelectorAll('.main-tab').forEach(function (el) { el.classList.toggle('active', el.dataset.panel === id); });
    document.querySelectorAll('.panel').forEach(function (el) { el.classList.toggle('active', el.id === id); });
}

function showSubPanel(id) {
    document.querySelectorAll('.subtab').forEach(function (el) { el.classList.toggle('active', el.dataset.subpanel === id); });
    document.querySelectorAll('.subpanel').forEach(function (el) { el.classList.toggle('active', el.id === id); });
}
...
function filterVisibleText(input) {
...
<div class="window">
    ...
    <div class="workspace">
        ...
        <div class="viewer">
            <div class="toolbar">
                <button class="toolbtn">Tools ▾</button>
                <input class="searchbox" type="search" value="<?= h(pathinfo($filePath, PATHINFO_FILENAME)) ?>" oninput="filterVisibleText(this)" />
                <span class="package-name small"><?= h($filePath) ?> (<?= h($pkg->getFileSize()) ?>)</span>
            </div>

            <div class="main-tabs">
                <button class="main-tab active" data-panel="package-panel" onclick="showPanel('package-panel')">▣ Package</button>
                <button class="main-tab" data-panel="content-panel" onclick="showPanel('content-panel')">▤ Content</button>
                <button class="main-tab" data-panel="externs-panel" onclick="showPanel('externs-panel')">⌘ Externs</button>
                <button class="main-tab" data-panel="tables-panel" onclick="showPanel('tables-panel')">▦ Tables</button>
            </div>
            ...
            <section id="package-panel" class="panel active">
                <div class="package-grid">
                    ...
                    <div class="field-grid">
                        ...
                        <div class="field-label">Folder</div><div class="field-value"></div>
                ...
                <table class="flag-table">
                    ...
                    <?php endforeach; ?>
                    <?php if (!$pkgFlagsDecoded): ?><tr><td></td><td></td></tr><?php endif; ?>
                    </tbody>
                </table>

                <?php if (!empty($issues)): ?>
                <div class="warn"><strong>Validation</strong><ul><?php foreach ($issues as $w): ?><li class="mono wrap"><?= h($w) ?></li><?php endforeach; ?></ul></div>
                <?php endif; ?>
            </section>
                ...
                <div class="content-list">
                    <?php foreach ($exports as $i => $ex): ?>
                        <div class="content-item" data-filter-row>
                            <span class="mono"><?= h($pkg->exportObjectName((int)($ex['objectName'] ?? -1))) ?></span>
                            <span class="content-class mono"><?= h($pkg->exportClassName((int)($ex['classIndex'] ?? 0))) ?></span>
                        </div>
                    <?php endforeach; ?>
                </div>
            </section>
            ...
            <section id="externs-panel" class="panel">
                <div class="tree">
                    <?php foreach (($importsByOuter[0] ?? []) as $rootIdx): ?>
                        <?php $renderImportNode((int)$rootIdx); ?>
                    <?php endforeach; ?>
                    <?php foreach (($exportsByOuter[0] ?? []) as $rootExportIdx): ?>
                        <?php $renderExportNode((int)$rootExportIdx); ?>
                    <?php endforeach; ?>
                </div>
            </section>
            ...
            <section id="tables-panel" class="panel">
                <div class="subtabs">
                    <button class="subtab active" data-subpanel="names-subpanel" onclick="showSubPanel('names-subpanel')">☰ Names</button>
                    <button class="subtab" data-subpanel="exports-subpanel" onclick="showSubPanel('exports-subpanel')">▤ Exports</button>
                    <button class="subtab" data-subpanel="imports-subpanel" onclick="showSubPanel('imports-subpanel')">▧ Imports</button>
                    <button class="subtab" data-subpanel="generations-subpanel" onclick="showSubPanel('generations-subpanel')">☷ Generations</button>
                </div>

                <div id="names-subpanel" class="subpanel active">
                    ...
                    <table class="data names-table">
                        ...
                        <?php foreach ($names as $n): ?>
                            ...
                            <tr data-filter-row>
                                ...
                                <td class="mono wrap"><?= h(hx($flags)) ?><?= h(flags_text($pkg->decodeRF($flags))) ?></td>
                                <td class="mono"><?= h(($n['index'] ?? '') . ' (' . hx2((int)($n['index'] ?? 0)) . ')') ?></td>
                                <td class="small raw-cell"><?= h($n['name'] ?? '') ?> / <?= h($flags) ?></td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
                    </table>
                ...
                <div id="exports-subpanel" class="subpanel">
                    ...
                    <table class="data exports-table">
                        <thead><tr><th>Class</th><th>Super</th><th>Package</th><th>Object</th><th>Object Flags</th><th>Size</th><th>Offset</th><th>Num.</th><th>Properties</th><th class="small">Raw</th></tr></thead>
                        <tbody>
                        <?php foreach ($exports as $i => $ex): ?>
                            <?php
                            ...
                            <tr data-filter-row>
                                ...
                                <td class="mono wrap"><?= h(hx($objectFlags)) ?><?= h(flags_text($pkg->decodeRF($objectFlags))) ?></td>
                                <td class="mono"><?= h($ex['serialSize'] ?? '') ?></td>
                                <td class="mono"><?= h(hx((int)($ex['serialOffset'] ?? 0))) ?></td>
                                <td class="mono"><?= h($i . ' (' . hx2($i) . ')') ?></td>
                                <td>
                                    <?php if ($hasProps): ?>
                ...
                <div id="imports-subpanel" class="subpanel">
                    ...
                    <table class="data imports-table">
                        <thead><tr><th>Class Package</th><th>Class Name</th><th>Package Name</th><th>Object Name</th><th>Num.</th><th class="small">Raw</th></tr></thead>
                        <tbody>
                ...
                <div id="generations-subpanel" class="subpanel">
                    <h2>Generations (<?= count($hdr['generations'] ?? []) ?>)</h2>
                    <table class="data">
                        <thead><tr><th>ExportCount</th><th>NameCount</th><th>Num.</th><th class="small">Raw</th></tr></thead>
                        <tbody>
                            <?php foreach (($hdr['generations'] ?? []) as $i => $g): ?>
                            <tr><td><?= h($g['e'] ?? '') ?></td><td><?= h($g['n'] ?? '') ?></td><td><?= h($i) ?></td><td class="small raw-cell"><?= h(($g['e'] ?? '') . ' / ' . ($g['n'] ?? '')) ?></td></tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                </div>
