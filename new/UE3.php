 ...
 * Purpose: Supports the standalone `new` parser/viewer workflow for UE3.
...
require_once __DIR__ . '/UnrealPackageReader5.php';

// Accept ?file=... (or set a default)
$filePath = isset($_GET['file']) ? (string)$_GET['file'] : (isset($filePath) ? (string)$filePath : 'testde.ut3');
...
<tr><th>Name</th><th>Value</th><th class="small">Raw</th></tr>
    ...
    <tbody>
		...
		<?php if (($hdr['version']??0) >= 249): ?>
		<tr><th>HeaderSize</th><td class="mono">             <?= h($hdr['headerSize'] ?? '') ?></td><td class="small"><?= h($hdr['headerSize'] ?? '') ?></td></tr>
        <?php endif; ?>	
		<?php if (($hdr['version']??0) >= 269): ?>
		<tr><th>FolderName</th><td class="mono">             <?= h($hdr['folderName'] ?? '') ?></td><td class="small"><?= h($hdr['folderName'] ?? '') ?></td></tr>
        <?php endif; ?>			
		...
		<?php if (($hdr['version']??0) >= 415): ?>
		<tr><th>DependsOffset</th><td class="mono">             <?= h($hdr['dependsOffset'] ?? '') ?></td><td class="small"><?= h($hdr['dependsOffset'] ?? '') ?></td></tr>
        <?php endif; ?>			
		...
		<?php endif; ?>	
		<?php if (isset($hdr['engineVersion'])): ?>
		<tr><th>EngineVersion</th><td class="mono">             <?= h($hdr['engineVersion'] ?? '') ?></td><td class="small"><?= h($hdr['engineVersion'] ?? '') ?></td></tr>
		<?php endif; ?>	
		<?php if (isset($hdr['cookerVersion'])): ?>
		<tr><th>CookerVersion</th><td class="mono">             <?= h($hdr['cookerVersion'] ?? '') ?></td><td class="small"><?= h($hdr['cookerVersion'] ?? '') ?></td></tr>
        <?php endif; ?>					
		<?php if (isset($hdr['cFlags'])): 	
		$flags = (int)($hdr['cFlags'] ?? $hdr['compressionFlags'] ?? 0); ?>
		<tr><th>CompressionFlags</th><td class="mono"><?= h(sprintf('0x%08X', $flags) ?? '')." (".$pkg->describeCompressionFlags($flags).")" ?></td><td class="small"><?= h($hdr['cFlags'] ?? '') ?></td></tr>
		<?php endif; ?>			
    </tbody>
</table>
	<?php
	...
	<table>
	  ...
	  <tr><th>Total (compressed → uncompressed)</th><td class="mono"><?= h($comp['totalCompressed']) ?> → <?= h($comp['totalUncompressed']) ?> bytes</td></tr>	  		  
	</table>
	
	
		<?php if (!empty($hdr['compressedChunks'])): ?>
	<h3>Chunks 2 (<?= count($hdr['compressedChunks']) ?>)</h3>
	<table>
	  <thead><tr><th>#</th><th>Comp Off/Len</th><th>Uncomp Off/Len</th></tr></thead>
	  <tbody>
		<?php foreach ($hdr['compressedChunks'] as $ci => $c): ?>
		<tr>
...
<?php foreach ($hdr['generations'] as $i=>$g): ?>
        ...
        <tr><td><?=$g['exportCount']?></td><td><?=$g['nameCount']?></td><td><?=$g['netObjectCount']?></td><td><?=$i?></td><td class="small"><?=$g['exportCount'] ?> / <?= $g['nameCount']?> / <?=$g['netObjectCount']?></td></tr>
	    <?php else: ?>
        <tr><td><?=$g['exportCount']?></td><td><?=$g['nameCount']?></td><td>&nbsp;</td><td><?=$i?></td><td class="small"><?=$g['exportCount'] ?> / <?= $g['nameCount']?></td></tr>
        <?php endif; ?>

<?php endforeach; ?>
</table>
<?php endif; /* 7001000000000 */ ?>
...
<table>
	...
	<?php $pkgFlagsDecoded = $pkg->decodePKG(intval($n['objectFlags'] ?? 0)); ?>
        <tr>
            <td><?= h($n['name']) ?></td>
            <td class="mono"><?=  h(sprintf("0x%08X",$n['flags'])) ?></td>			
...
<table>
    ...
    <?php foreach ($imports as $i => $im): ?>
		...
		//print_r($im);
		//echo "</pre>";
		//h($pkg->importOuterName((int)$i)) 
		?>
		    <td><?= h($im['ClassPackage']['text']) ?></td>
		    <td><?= h($im['ClassName']['text']) ?></td>
		    <td><?= h($im['outerName']) ?></td>
		    <td><?= h($im['ObjectName']['text']) ?></td>
            <td class="mono"><?= h($i." (".sprintf("0x%02X",$i)) ?>)</td>
            <td class="mono small"><?= h(($im['ClassPackage']['index'] ?? '') . ' / ' . ($im['ClassName']['index'] ?? '') . ' / ' . ($im['OuterIndex']  ?? '') . ' / ' . ($im['ObjectName']['index'] ?? '')) ?></td>
        </tr>
    <?php endforeach; ?>
    </tbody>
</table>
...
<?php $sup = $ex['superIndex'] ?? null; ?>
<td class="mono">
  <?= $sup === 0 || $sup === '0' ? '0' : h((string)$sup) ?>
</td>
		
<?php
$pkgRef  = $ex['packageIndex'] ?? $ex['outerIndex'] ?? null;
$pkgText = ($pkgRef === 0 || $pkgRef === '0' || $pkgRef === null) ? 'None'  : $pkg->exportPackageName((int)$pkgRef);
?>
<td class="mono">
  <?= h((string)($pkgRef ?? '')) ?>
  <?php if ($pkgRef): ?>
    <small>(<?= h($pkgText) ?>)</small>
  <?php endif; ?>
</td>			
...
<?php if (!empty($ex['archetype']) || !empty($ex['packageFlags']) || !empty($ex['components'])): ?>

  <div class="small mono" style="margin-top:4px">
    <?php if (!empty($ex['archetype'])): ?>
      Archetype: <?= h($pkg->displayNameFromRef((int)$ex['archetype'])) ?>
      (ref <?= (int)$ex['archetype'] ?>)<br/>
    <?php endif; ?>
    <?php if (!empty($ex['packageFlags'])): ?>
      Export PackageFlags: <?= h(sprintf("0x%08X",(int)$ex['packageFlags'])) ?><br/>
    <?php endif; ?>
    <?php if (!empty($ex['components'])): ?>
      Components: 
      <?php
        $c = [];
        foreach ($ex['components'] as $cmp) {
          $c[] = $pkg->nameByIndex((int)$cmp['name']) . ' → ' . $pkg->displayNameFromRef((int)$cmp['ref']);
        }
        echo h(implode(', ', $c));
      ?>
    <?php endif; ?>
