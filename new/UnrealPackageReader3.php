 ...
 * Purpose: Provides `new` parser/viewer support for unreal package reader3, including `UEReader`.
...
// Add in UnrealPackageReader.php (class body)
private static function decodePackageFlags(int $flags): array {
    // Common UE1/UE2 flags (not exhaustive; safe set)
    $map = [
        0x00000001 => 'AllowDownload',
        0x00000002 => 'ClientOptional',
        0x00000004 => 'ServerSideOnly',
        0x00000008 => 'Cooked',
        0x00008000 => 'Unsecure',        // appears in some branches
        0x02000000 => 'Stripped',        // editor-only stripped data
        0x04000000 => 'Compressed',      // generic compressed indicator
        0x10000000 => 'ContainsDebug',   // debug symbols or names
    ];
    $out = [];
    foreach ($map as $bit => $label) {
        if (($flags & $bit) !== 0) $out[] = $label;
    }
    return $out;
}

public function getPackageFlagsDecoded(): array {
    $flags = (int)($this->header['pkgFlags'] ?? 0);
    return [
        'value' => $flags,
        'hex'   => sprintf('0x%08X', $flags),
        'set'   => self::decodePackageFlags($flags),
    ];
}
