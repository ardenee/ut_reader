 ...
 * Purpose: Provides `new` parser/viewer support for unreal package reader5a, including `UEReader`.
...
final class UnrealPackageReader
{
    ...
    private const PKG_FLAGS = [
        // UE1 / UE2 / UE3 combined — names follow the most common usage
        0x00000001 => 'PKG_AllowDownload',   // Allow downloading package
        0x00000002 => 'PKG_ClientOptional',  // Purely optional for clients
        0x00000004 => 'PKG_ServerSideOnly',  // Only needed on server side
        0x00000008 => 'PKG_BrokenLinks',     // UE1: loaded with broken import links | UE2+: NoExportAllowed
        0x00000010 => 'PKG_Unsecure',        // UE1: not trusted | UE2+: Cooked
        0x00000020 => 'PKG_Encrypted',       // UE3+: package is encrypted
        0x00008000 => 'PKG_Need',            // UE1: client needs to download this package
    ];
...
private function disasmScript(UEReader $R, int $limit = 4096): array {
	...
	*/	
	...
	public function describeCompressionFlags(int $flags): string
	{
		...
		if (empty($parts))
			return $flags ? sprintf('0x%04X', $flags) : 'None';
	...
	{
		// UE3 uses a named-type tag format (type as Name string, not a nibble).
		// The UE3 tag format starts at approximately version 300.  Versions 220-299
		// are UE2.5 licensee builds that still use the UE1/UE2 nibble format,
		// so the split point is 300, not 220.
		if (($this->header['version'] ?? 0) >= 300) {
...
*/
    ...
    private function readNameTable(): void
	{
		...
		if ($version >= 300 && $this->isCompressed) {
		...
		for ($i = 0; $i < $count; $i++) {
			if ($version < 300) {  // UE1 / UE2 — NAME type (ASCIIZ or length-prefixed)
				$nameStr = $this->readNAME($R, $version);
				// UE1/UE2: object flags are always DWORD (4 bytes).
				// Only UE3 (version >= 300) stores them as QWORD.
				$flags   = $R->u32();
				...
				$flags = $R->u64(); // UE3 name flags are QWORD
			}
	...
	private function readImportTable(): void
	{
		...
		for ($i=0; $i<$count; $i++) {
			$classPackage = $R->index();   // INDEX → name table
			$className    = $R->index();   // INDEX → name table
			$outerIndex   = $R->i32();     // DWORD object ref (0/±)
			$objectName   = $R->index();   // INDEX → name table

			$imports[] = [
				// --- Raw fields (backward-compatible keys, camelCase) ---
				'classPackage' => $classPackage,
				'className'    => $className,
				'outerIndex'   => $outerIndex,
				'objectName'   => $objectName,
				// --- Enriched fields expected by the view (PascalCase) ---
				// Text strings are filled in the post-pass below, once all
				// imports are in $this->imports so cross-references work.
				'ClassPackage' => ['index' => $classPackage, 'text' => ''],
				'ClassName'    => ['index' => $className,    'text' => ''],
				'OuterIndex'   => $outerIndex,
				'outerName'    => '',   // resolved in post-pass
				'ObjectName'   => ['index' => $objectName,   'text' => ''],
			];
		}
		
		// Commit imports so displayNameFromRef() can walk the outer chain.
		$this->imports = $imports;

		// Post-pass: resolve all name strings now that the full import list is
		// available (import outers always reference earlier imports, so this is safe).
		foreach ($this->imports as $i => &$im) {
			$im['ClassPackage']['text'] = $this->nameByIndex($im['classPackage']) ?? '';
			$im['ClassName']['text']    = $this->nameByIndex($im['className'])    ?? '';
			$im['ObjectName']['text']   = $this->nameByIndex($im['objectName'])   ?? '';

			$outerRef      = (int)$im['outerIndex'];
			$im['outerName'] = ($outerRef !== 0)
				? $this->displayNameFromRef($outerRef)
				: '';
		}
		unset($im); // break reference
	}
	...
	private function readExportTable(): void
	{
		...
		for ($i=0; $i<$count; $i++) {
			...
			$packageIndex = $R->i32();    // ref: 0/±  (DWORD in all versions)
			$objectName   = $R->index();  // name index
			// Object flags: DWORD in UE1/UE2, QWORD in UE3 (version >= 300)
			$objectFlags  = ($version >= 300) ? $R->u64() : $R->u32();
			$serialSize   = $R->index();
...
final class UEReader
{
    ...
    public function index(): int {
        // UE3 (version >= 300) abandoned the variable-length CompactIndex and
        // stores all indices as plain little-endian int32.
        // UE1 (< 117) and UE2 (117-~270) both use the 1-to-5-byte compact format.
        if ($this->version >= 300) {			
            return $this->i32();
        }
