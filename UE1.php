...
function row_raw(array $row): array { return isset($row['raw']) && is_array($row['raw']) ? $row['raw'] : $row; }
function row_view(array $row): array { return isset($row['view']) && is_array($row['view']) ? $row['view'] : []; }
...
if ($filePath === '') {
    foreach ([__DIR__ . '/oldtest.utx', __DIR__ . '/test.utx', __DIR__ . '/test.u', __DIR__ . '/test.unr'] as $candidate) {
...
if ($filePath !== '') {
    try {
        ...
        if (!is_readable($filePath)) throw new RuntimeException('File is not readable by PHP/Web Station: ' . $filePath);
...
<?php if (isset($hdr['heritageCount'])): ?><tr><th>Heritage count</th><td class="mono"><?=h($hdr['heritageCount'])?></td><td class="small"><?=h($hdr['heritageCount'])?></td></tr><?php endif; ?>
<?php if (isset($hdr['heritageOffset'])): ?><tr><th>Heritage offset</th><td class="mono"><?=h($hdr['heritageOffset'])?></td><td class="small"><?=h($hdr['heritageOffset'])?></td></tr><?php endif; ?>
</tbody></table>
...
<tr><td><?=h($g['exportCount'] ?? $g['e'] ?? '')?></td><td><?=h($g['nameCount'] ?? $g['n'] ?? '')?></td><td class="mono"><?=h($i)?></td><td class="small mono"><?=h(json_encode($g))?></td></tr>
<?php endforeach; ?>
</tbody></table>
<?php endif; ?>

<h2>Names (<?=count($names)?>)</h2>
<table><thead><tr><th>Name</th><th>Flags</th><th>Num.</th><th class="small">Raw</th></tr></thead><tbody>
...
<table><thead><tr><th>Class Package</th><th>Class Name</th><th>Package Name</th><th>Object Name</th><th>Num.</th><th class="small">Raw refs</th></tr></thead><tbody>
...
?>
<tr><td><?=h(name_at($names,$classPkgIdx))?></td><td><?=h(name_at($names,$classIdx))?></td><td><?=h(object_ref_path($outerRef,$imports,$exports,$names))?></td><td><?=h(name_at($names,$objIdx))?></td><td class="mono"><?=h($i . ' (' . hx($i) . ')')?></td><td class="mono small"><?=h($classPkgIdx . ' / ' . $classIdx . ' / ' . $outerRef . ' / ' . $objIdx)?></td></tr>
<?php endforeach; ?>
</tbody></table>

<h2>Exports (<?=count($exports)?>)</h2>
<table><thead><tr><th>Class Index</th><th>Super Index</th><th>Package Index</th><th>Object Name</th><th>Object Flags</th><th>Serial Size</th><th>Serial Offset</th><th>Num.</th><th>Properties</th><th class="small">Raw refs</th></tr></thead><tbody>
<?php foreach ($exports as $i => $ex):
    ...
    $props=[]; $propError=null;
    if (method_exists($pkg, 'getExportProperties')) {
        try { $props = (array)($pkg->getExportProperties($i) ?? []); } catch (Throwable $t) { $propError = $t->getMessage(); }
    }
    $hasProps=!empty($props) || $propError !== null;
?>
<tr>
<td class="mono"><?=h(object_ref_name($classRef,$imports,$exports,$names))?></td>
<td class="mono"><?=h(object_ref_name($superRef,$imports,$exports,$names))?></td>
<td class="mono"><?=h(object_ref_path($pkgRef,$imports,$exports,$names))?></td>
...
<td><?php if ($hasProps): ?><button type="button" class="toggle-btn" onclick="toggleProps(<?= (int)$i ?>)">Properties <span class="pill"><?=count($props)?></span></button><?php else: ?><span class="small mono">—</span><?php endif; ?></td>
...
<?php if ($props): ?><table class="nested"><thead><tr><th>Key</th><th>Value</th></tr></thead><tbody>
<?php foreach ($props as $pi => $p): ?><tr><td class="mono"><?=h($pi)?></td><td class="raw mono"><?=h(is_scalar($p) ? (string)$p : json_encode($p, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE))?></td></tr><?php endforeach; ?>
</tbody></table><?php endif; ?>
</div></td></tr>
<?php endif; ?>
<?php endforeach; ?>
</tbody></table>
<script>function toggleProps(i){const row=document.getElementById('props-'+i);if(!row)return;row.style.display=(row.style.display==='table-row')?'none':'table-row';}</script>
</body>
</html>
