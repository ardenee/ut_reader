...
function build_ue2_raw_header_fields(string $packagePath): array
{
    ...
    if ($version >= 415) {
        $dependsOffset = read_le_i32($bytes, $pos);
        add_raw_header_field($fields, $bytes, $pos, 4, 'dependsOffset', 'int32', $dependsOffset);
        $pos += 4;
    }
    ...
    add_unparsed_header_bytes($fields, $bytes, $pos, $headerEnd, 'Bytes between decoded UE2 header fields and the first package table.');
...
function renderRawHeaderFields(array $fields): void
{
    ...
    <details class="grid-after-tree raw-header-details">
        ...
        <table class="data raw-header-table">
            <thead>
                <tr>
                    <th>Offset</th><th>Size</th><th>Field</th><th>Type</th><th>Value</th><th>Raw Hex</th><th>Note</th>
                </tr>
            </thead>
            <tbody>
...
$rawHeaderFields = build_ue2_raw_header_fields($filePath);
...
function renderImportNode(UnrealPackageReader $pkg, array $imports, array $importsByOuter, int $idx, bool $includeChildren, string $idPrefix = ''): void
{
    ...
    <details id="<?= h($anchor) ?>" class="tree-node" data-filter-row>
        ...
        <div class="tree-lines">
            <div>Object:<?= name_link_html($pkg, $objectIdx, $object) ?><a class="ref-tag ref-link" href="#<?= h(object_ref_target_id($ref)) ?>" data-ref="<?= h($ref) ?>">(<?= h($ref) ?>)</a></div>
    ...
    <div class="workspace">
        ...
        <div class="viewer">
                ...
                <div class="file-open-bar">
                    <form method="get">
                        <select class="file-select" name="file">
                            ...
                            <?php endforeach; ?>
                            <?php if (is_file(__DIR__ . '/test.utx')): ?><option value="test.utx"<?= $currentRel === 'test.utx' ? ' selected' : '' ?>>test.utx</option><?php endif; ?>
                            <?php if (is_file(__DIR__ . '/oldtest.utx')): ?><option value="oldtest.utx"<?= $currentRel === 'oldtest.utx' ? ' selected' : '' ?>>oldtest.utx</option><?php endif; ?>
                        </select>
            ...
            <section id="package-panel" class="panel active">
                ...
                <?php if (!empty($comp['isCompressed']) || !empty($comp['chunks'])): ?><h2>Compression</h2><table class="data"><tr><th>Compressed?</th><td class="mono"><?= !empty($comp['isCompressed']) ? 'Yes' : 'No' ?></td></tr><tr><th>Total compressed / uncompressed</th><td class="mono"><?= h($comp['totalCompressed'] ?? 0) ?> / <?= h($comp['totalUncompressed'] ?? 0) ?></td></tr></table><?php endif; ?>
