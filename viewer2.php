...
if (!empty($_FILES['file']['tmp_name'])) {
    if (!is_dir($uploadDir) && !mkdir($uploadDir, 0775, true)) {
        ...
        if (!move_uploaded_file($_FILES['file']['tmp_name'], $file)) {
            $uploadError = 'Unable to move uploaded file to: ' . $file;
            $file = null;
        }
    }
}
...
if (!$file) {
    foreach ([__DIR__ . '/test.ut3', __DIR__ . '/test.utx'] as $candidate) {
...
<style>
:root{--bg:#0f1116;--panel:#161a22;--muted:#8892a6;--outline:#95b8ff;--accent:#66aaff;--border:#252b36;--hi:#dfe7ff;--err:#ff9f9f}*{box-sizing:border-box}body{margin:0;font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;background:var(--bg);color:#e8eefc}header{padding:1rem 1.25rem;border-bottom:1px solid var(--border);display:flex;gap:1rem;align-items:center;flex-wrap:wrap}h1{margin:0;font-weight:700;color:var(--outline)}form{display:flex;gap:.75rem;align-items:center;flex-wrap:wrap}input[type=file]{color:#e8eefc}input[type=submit],select{background:var(--panel);color:#e8eefc;border:1px solid var(--border);padding:.5rem .75rem;border-radius:.5rem;cursor:pointer}main{padding:1rem;display:grid;grid-template-columns:320px 1fr;gap:1rem}.card{background:var(--panel);border:1px solid var(--border);border-radius:12px;overflow:hidden}.card h2{margin:0;padding:.85rem 1rem;border-bottom:1px solid var(--border);color:var(--outline)}.body{padding:.85rem 1rem}.wide{grid-column:1/-1}.table-wrap{overflow:auto;max-height:65vh}table{width:100%;border-collapse:collapse;font-size:.95rem}th,td{padding:.5rem .6rem;border-bottom:1px solid var(--border);vertical-align:top;text-align:left;white-space:nowrap}tr:hover td{background:#141923}.mono{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,"Liberation Mono",monospace}.wrap{white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word}.dim{opacity:.65;font-size:.9em}.err{color:var(--err);background:#2a1116;border:1px solid #55303a;border-radius:8px;padding:.75rem;margin:.5rem 0}.controls{display:flex;gap:.5rem;align-items:center;flex-wrap:wrap;padding:.5rem 0}.controls input{flex:1 1 360px;min-width:220px;background:#0f131b;border:1px solid var(--border);padding:.6rem .75rem;border-radius:.75rem;color:#e8eefc}.row-click{cursor:pointer}.props-row{display:none}.badge{display:inline-block;padding:.15rem .45rem;border:1px solid var(--border);border-radius:.5rem;color:var(--hi);background:#121620;margin-right:.35rem}
</style>
</head>
<body>
<header>
...
<main>
  ...
  <div class="card"><h2>Names</h2><div class="body">
    ...
    <?php foreach ($names as $i => $row): $row=(array)$row; $raw=row_raw($row); $idx=(int)($row['index'] ?? $i); ?>
  ...
  <div class="card wide"><h2>Exports</h2><div class="body">
    ...
    <?php foreach ($exports as $i => $e):
        ...
        if ($class === '') $class = object_ref_name($classRef,$imports,$exports,$names);
        $super=object_ref_name($superRef,$imports,$exports,$names);
        $group='';
        if (!empty($view['outerChain']) && is_array($view['outerChain'])) $group=implode('.',array_filter($view['outerChain']));
        if ($group === '') $group=object_ref_path($pkgRef,$imports,$exports,$names);
  ...
  <div class="card wide"><h2>Imports</h2><div class="body">
    ...
    <?php foreach ($imports as $i => $im):
        ...
        $outer=object_ref_path($outerRef,$imports,$exports,$names);
