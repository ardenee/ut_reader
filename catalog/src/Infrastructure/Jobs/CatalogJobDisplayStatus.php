...
{
    private const FAILED_OUTCOMES = ['failed', 'rejected', 'invalid_ue_package'];
    ...
    public static function filterCondition(string $status, string $alias = ''): array
    {
        ...
        if ($status === 'failed') {
            return [
                'sql' => $prefix . 'display_status IN ("failed","rejected","invalid_ue_package")',
                'params' => [],
            ];
        }
        if ($status === 'completed') {
            return [
                'sql' => $prefix . 'status="completed" AND '
                    . $prefix . 'display_status NOT IN ("failed","rejected","invalid_ue_package") AND NOT ('
