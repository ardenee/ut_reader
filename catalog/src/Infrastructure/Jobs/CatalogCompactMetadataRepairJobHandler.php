<?php
/**
 * Repairs one verified file's current-format metadata from its authoritative stored
    ...
    public function handle(ClaimedJob $job, JobExecutionContext $context): array
    {
        ...
        if ($stage === 'compact_repair_reimport') {
            $context->heartbeatIfDue($this->progress(
                'compact_repair_reimport',
                20,
                'Rebuilding current-format metadata for ' . (string)$file['original_name'] . '.',
                ['file_id' => $fileId]
            ));
            ...
            $context->checkpoint($this->progress(
                'compact_repair_dependency_plan',
                55,
                'Current-format metadata repaired for ' . (string)$file['original_name']
