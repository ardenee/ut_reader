 ...
 * Role: Infrastructure PDO collaborator preserving the historical optimization table list, progress and warning behavior.
 */
...
final class PdoCatalogGameTableMaintenance
{
    ...
    public function optimiseTables(
        ...
        foreach ($existing as $index => $table) {
            $before = $startPercent
                + (int)floor((($endPercent - $startPercent) * $index) / $total);
            ...
            try {
                ...
                error_log(
                    '[UnrealDB game lifecycle] OPTIMIZE TABLE ' . $table
            ...
            foreach ($statement->fetchAll(PDO::FETCH_ASSOC) as $row) {
                ...
                if ($type === 'error') {
                    $reportedErrors[] = trim(
                        (string)($row['Msg_text']
                            ?? $row['msg_text']
                            ?? 'Unknown database optimisation error.')
                    );
                }
            }
