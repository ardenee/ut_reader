...
function gm_remove_storage_tree(string $targetPath, string $storageRoot): int
{
 ...
 */
function gm_reset_game_files(PDO $db, array $config, int $gameId): array
{
    $game = catalog_one(
        $db,
    ...
    $storedFilesRemoved = gm_remove_storage_tree($gameStoragePath, $storageRoot);

    $db->beginTransaction();
    try {
        $stmt = $db->prepare('DELETE FROM ue_files WHERE game_id=?');
        $stmt->execute([$gameId]);
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'reset_game_files') {
            ...
            $result = gm_reset_game_files($db, $config, $gameId);
            $_SESSION['game_manager_flash'] = 'Reset ' . $result['game_name'] . ': removed '
            ...
            header('Location: game-manager.php?game_id=' . (int)$result['game_id']);
            exit;
        }
...
CSS;
    ...
    if (!$games) {
        ...
        foreach ($games as $game) {
            ...
            echo '<tr><td><strong>' . catalog_h($game['name']) . '</strong><br><span class="muted small">' . catalog_h($game['slug']) . '</span></td><td>' . catalog_h($game['profile_name'] ?? 'none') . '</td><td><span class="pill ' . $engineClass . '">' . catalog_h($engine) . '</span></td><td class="mono small">' . catalog_h(is_array($exts) ? implode(', ', $exts) : '') . '</td><td class="mono">' . catalog_h($range) . '</td><td>' . $fileCount . '</td><td>' . (int)$game['source_count'] . '</td><td><div class="game-actions"><a class="button" href="game-manager.php?game_id=' . $gameId . '">Edit</a> <a class="button" href="sources.php?game_id=' . $gameId . '">Sources</a> <a class="button" href="profiled-upload.php?game_id=' . $gameId . '">Upload</a> <form method="post" class="game-reset-form" data-game-name="' . catalog_h($game['name']) . '" data-file-count="' . $fileCount . '"><input type="hidden" name="csrf" value="' . catalog_h(catalog_csrf('game_manager')) . '"><input type="hidden" name="action" value="reset_game_files"><input type="hidden" name="game_id" value="' . $gameId . '"><input type="hidden" name="confirm_reset" value="yes"><button type="submit" class="button game-reset-button">Reset</button></form></div></td></tr>';
        }
        echo '</table>';
    }
    echo '</div>';
...
document.querySelectorAll('.game-reset-form').forEach(function (form) {
    form.addEventListener('submit', function (event) {
        ...
        var message = 'Reset ' + gameName + '?\n\nThis will permanently delete all catalogued package files for this game, remove their database records, and clear related Names, Imports, Exports, locations, and dependency rows.\n\nGame setup, profile assignment, and source definitions will stay.\n\nCatalog records affected: ' + fileCount;
