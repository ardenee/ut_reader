...
{
    $file = catalog_one($db, 'SELECT id, game_id, package_name, original_name FROM ue_files WHERE id=?', [$fileId]);
    if (!$file) {
...
CSS;
    ...
    echo CatalogUi::pageHeader(
        ...
        'Repair existing catalog rows that still contain browser/Windows filename suffixes in package names, original filenames, or export full paths.',
        ['Duplicates' => 'duplicates.php', 'Full Sync' => 'full-sync.php', 'Games' => 'games.php']
    );

    echo '<div class="card"><h2>What this fixes</h2><p>Legacy Unreal package headers do not provide a separate catalog package-root field, so older imports could preserve browser-renamed filenames such as <span class="mono">of1 (2).utx</span>. This maintenance page rewrites catalog metadata to the clean logical root, for example <span class="mono">of1</span>, and rewrites exports from <span class="mono">of1 (2).Palette3</span> to <span class="mono">of1.Palette3</span>.</p><p class="muted">The stored package file remains hash-named. This only repairs catalog metadata and dependency matching.</p></div>';

    echo '<section class="ui-section"><div class="ui-section__header"><div><h2>Filter</h2><p>Shows up to ' . PACKAGE_NORMALIZE_MAX_ROWS . ' dirty rows per run.</p></div></div><div class="ui-section__body">';
    echo '<form class="package-normalize-controls" method="get"><label for="game_id">Game<select id="game_id" name="game_id"><option value="0">All games</option>';
    ...
    if ($dirtyRows === []) {
        echo CatalogUi::emptyState('No dirty package roots found', 'No package names, original filenames, or export full paths need normalization for the selected filter.');
        catalog_foot();
        exit;
    }

    echo '<form method="post" onsubmit="return confirm(\'Normalize selected package roots and export full paths?\')">';
