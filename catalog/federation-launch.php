...
try {
    ...
    $stats = [
        ...
        'queued' => (int)(catalog_one($db, 'SELECT COUNT(*) c FROM ue_federation_transfer_jobs j WHERE j.status="queued" AND ' . $visibleJobSql)['c'] ?? 0),
        'downloaded' => (int)(catalog_one($db, 'SELECT COUNT(*) c FROM ue_federation_transfer_jobs j WHERE j.status="downloaded" AND ' . $visibleJobSql)['c'] ?? 0),
        'failed' => (int)(catalog_one($db, 'SELECT COUNT(*) c FROM ue_federation_transfer_jobs j WHERE j.status="failed" AND ' . $visibleJobSql)['c'] ?? 0),
        'requests' => (int)(catalog_one(
            $db,
    ...
    echo '<div class="stat"><h2>' . $stats['peer_files'] . '</h2><p>Policy-visible inventory rows</p></div>';
    echo '<div class="stat"><h2>' . $stats['requests'] . '</h2><p>Policy-visible file requests</p></div>';
    echo '<div class="stat"><h2>' . $stats['queued'] . '</h2><p>Policy-visible queued jobs</p></div>';
    echo '<div class="stat"><h2>' . $stats['downloaded'] . '</h2><p>Policy-visible waiting import</p></div>';
    echo '<div class="stat"><h2>' . $stats['failed'] . '</h2><p>Policy-visible failed jobs</p></div>';
    echo '</div>';
    ...
    fl_card('Missing Files', 'federation/missing-files.php', 'Select policy-eligible locally missing dependency packages.');
    fl_card('Requests', 'federation/request-center.php', 'Policy-filtered role-aware request overview.');
    fl_card('Incoming File Requests', 'federation/requests.php', 'Parent-side approval of policy-eligible files requested by children.');
    fl_card('Outgoing File Requests', 'federation/request-status.php', 'Child-side status for policy-visible requests sent to a parent.');
    fl_card('Approved Downloads', 'federation/approved-downloads.php', 'Child-side policy-visible approved download and import history.');
    echo '</div></div>';

    echo '<div class="card"><h2>Inventories and transfers</h2><div class="grid">';
    fl_card('Child Inventories', 'federation/peer-inventory.php', 'Parent-side policy-filtered child selection and three need views.');
    fl_card('Parent Pull', 'federation/parent-pull.php', 'Parent pulls policy-eligible selected files directly from children.');
    ...
    fl_card('Conflicts', 'federation/conflicts.php', 'Review policy-visible GUID, package, and hash conflicts.');
    echo '</div></div>';

    echo '<div class="card"><h2>Administration</h2><div class="grid">';
    fl_card('Federation Overview', 'federation/admin.php', 'Role-aware policy-filtered dashboard and next actions.');
