...
function pak_import_queue_failed(PDO $db, array $config, array $game, array $file, string $display, Throwable $error, ?int $userId): void
{
    $path = (string)$file['path'];
    if (!is_file($path) || !scanner_file_has_unreal_package_magic($path)) return;
    $queueGame = ['id'=>(int)$game['id'],'name'=>(string)$game['name'],'slug'=>(string)$game['slug'],'profile_id'=>null];
    $dir = uvf_unverified_dir($config,$queueGame,true);
    $originalName = catalog_clean_unreal_filename(basename($display));
    $queueName = basename(uvf_unique_destination($dir,uvf_safe_queue_name($originalName)));
    $destination = $dir . DIRECTORY_SEPARATOR . $queueName;
    if (!@rename($path,$destination)) throw new RuntimeException('Could not move failed PAK entry into unverified storage.');
    $reason = 'PAK entry ' . $display . ': ' . pak_import_short_error($error);
    @file_put_contents($destination . '.txt',$reason);
    try {
        catalog_unverified_index_path($db,$config,(int)$game['id'],$queueName,$destination,$originalName,$reason,$userId,$display,false);
...
function pak_import_handle_request(PDO $db, array $config): array
{
    ...
    $gameId=(int)($_POST['game_id']??0);$strictProfile=(string)($_POST['strict_profile']??'1')==='1';
    ...
    $found=count($extracted['files']);$scannable=$imported=$duplicates=$aliases=$failed=$skipped=0;$samples=[];
    try{
        foreach($extracted['files'] as $file){
            $display=pak_import_relative_display($file);
            if(!pak_import_is_scannable_inner_file($file,$allowed)){$skipped++;continue;}
            $scannable++;
            try{[$display,$result]=pak_import_scan_extracted_file($db,$config,$gameId,$file,$strictProfile,$userId);$status=(string)($result[0]??'verified');if($status==='duplicate')$duplicates++;elseif($status==='alias')$aliases++;else$imported++;if(count($samples)<100)$samples[]=[$status,$display,(string)($result[2]??'')];}
            catch(Throwable $error){$failed++;if(count($samples)<100)$samples[]=['unverified',$display,pak_import_short_error($error)];pak_import_queue_failed($db,$config,$game,$file,$display,$error,$userId);}
        }
    }finally{catalog_pak_archive_delete_tree((string)$extracted['dir']);if(!empty($source['uploaded'])&&is_file((string)$source['path']))@unlink((string)$source['path']);}
...
try{
    $config=catalog_config();$db=catalog_db($config);if(!catalog_require_admin_page('PAK Import'))exit;
    ...
    if(is_array($result)){
        ...
        if($result['samples']){echo '<h3>Samples</h3><table><tr><th>Status</th><th>Path</th><th>Message</th></tr>';foreach($result['samples'] as $sample)echo '<tr><td><span class="pill">'.catalog_h((string)$sample[0]).'</span></td><td class="mono path">'.catalog_h((string)$sample[1]).'</td><td class="mono path">'.catalog_h((string)$sample[2]).'</td></tr>';echo '</table>';}
        echo '</div></section>';
    }
    ...
    echo '<p><label>Target game<br><select name="game_id" required><option value="">Choose target game</option>';foreach($games as $game)echo '<option value="'.(int)$game['id'].'"'.((int)$game['id']===$selectedGameId?' selected':'').'>'.catalog_h((string)$game['name'].' / '.((string)($game['profile_engine']?:'no profile'))).'</option>';
    echo '</select></label></p><p><label>Profile mismatch handling<br><select name="strict_profile"><option value="1" selected>Strict: move mismatches to unverified</option><option value="0">Loose: allow parser attempt</option></select></label></p><p><label>Upload .pak<br><input type="file" name="pak_file" accept=".pak"></label></p><p><label>Or local .pak path<br><input type="text" name="local_pak_path" style="width:min(100%,760px)"></label></p><p><button type="submit">Extract PAK and import contents</button></p></form></div></section>';
    catalog_foot();
}catch(Throwable $error){if(!headers_sent())catalog_head('PAK Import Error');echo CatalogUi::alert('danger',pak_import_short_error($error),'PAK import failed.');catalog_foot();}
