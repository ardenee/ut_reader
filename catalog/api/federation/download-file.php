...
try {
    ...
    if ((string)$peer['peer_role'] !== 'parent') {
        fed_json_response(['ok' => false, 'error' => 'Only a paired parent may pull files from this child.'], 403);
    }
    if ((string)fed_setting($db, 'allow_parent_pull_from_child', '1') !== '1') {
        fed_json_response(['ok' => false, 'error' => 'Parent pull is disabled on this child.'], 403);
    }
    ...
    fed_log($db, (int)$peer['id'], null, 'INFO', 'PARENT_PULL_DOWNLOAD', 'Serving file ID ' . $fileId . ' to parent peer.');
