 ...
 * Purpose: Provides shared catalog helper functions for catalog compact dependencies.
...
/**
 * Return true when the compact dependency schema is available.
 */
function catalog_compact_dependencies_available(PDO $db): bool
{
    ...
    return $cache[$key] = ((int)$statement->fetchColumn() === count($columns));
}

function catalog_compact_metadata_version(PDO $db, int $fileId): int
{
    if ($fileId < 1 || !catalog_compact_dependencies_available($db)) {
        return 0;
    }
...
{
    if (catalog_compact_metadata_version($db, $fileId) < 2) {
        return null;
    }
    ...
    $reader = new BlockedCompressedMetadataReader($db, $storagePath);
    $blockedByImport = [];
    $start = 0;
    $pageSize = 1000;
    do {
    ...
    foreach ($links as $link) {
        ...
        $rows[] = [
            ...
            'resolved_export_index' => $link['resolved_export_index'] !== null ? (int)$link['resolved_export_index'] : null,
            'status' => catalog_compact_dependency_status_label((int)$link['status']),
...
{
    $compact = catalog_compact_dependency_rows($db, $config, $fileId);
    if (is_array($compact)) {
        return $compact;
    }
...
function catalog_dependency_used_by_rows(PDO $db, int $targetFileId, int $limit = 200): array
{
    ...
    if (catalog_compact_dependencies_available($db)) {
        $statement = $db->prepare(
    ...
    $statement = $db->prepare($legacySql);
    $statement->execute([$targetFileId]);
    foreach ($statement->fetchAll(PDO::FETCH_ASSOC) ?: [] as $row) {
        $byFile[(int)$row['id']] = $row;
    }
    ...
    usort($rows, static function (array $left, array $right): int {
        $comparison = strnatcasecmp((string)$left['package_name'], (string)$right['package_name']);
        return $comparison !== 0
            ? $comparison
...
function catalog_reverse_dependency_rows(
    ...
    $identityNames = catalog_compact_unique_strings($identityNames);
    $rows = [];
    ...
    if (catalog_compact_dependencies_available($db)) {
        ...
        $reader = $compactRows !== [] ? new BlockedCompressedMetadataReader($db, $storagePath) : null;
        $indexesByFile = [];
        ...
        foreach ($compactRows as $row) {
            ...
            $row['status'] = catalog_compact_dependency_status_label((int)$row['status']);
            $rows[] = $row;
        }
    }
