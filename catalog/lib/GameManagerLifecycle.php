 ...
 * Purpose: Provides shared catalog helper functions for game manager lifecycle.
...
function gm_lifecycle_optimise_tables(
    ...
    foreach ($existing as $index => $table) {
        ...
        gm_emit($progress, 'optimise', $index, $total, $before, 'Optimising database table ' . ($index + 1) . '/' . $total . ': ' . $table);
        try {
    ...
    gm_emit(
        ...
        $total,
        $endPercent,
        $failed === [] ? 'Database table optimisation complete.' : 'Database optimisation completed with ' . count($failed) . ' warning(s).'
    );
...
function gm_lifecycle_unverified_rows(PDO $db, int $gameId): array
{
    return array_map(
        ...
        catalog_all(
            ...
            . 'WHERE game_id IS NULL AND scan_status="unverified" AND unverified_queue_game_id=? ORDER BY id',
            [$gameId]
        )
    );
}
...
function gm_lifecycle_cleanup_game(
    ...
    if (gm_lifecycle_table_exists($db, 'ue_pak_archives')) {
        $pak = catalog_one(
            $db,
            'SELECT COUNT(*) archive_count,COALESCE(SUM(file_size),0) total_size FROM ue_pak_archives WHERE game_id=?',
            [$gameId]
        ) ?: [];
    ...
    $mappedProgress = $progress === null ? null : static function (array $state) use ($progress, $startPercent, $innerEnd): void {
        ...
        gm_emit(
            ...
            $startPercent + (int)floor((($innerEnd - $startPercent) * $innerPercent) / 100),
            (string)($state['message'] ?? 'Removing game files…')
        );
    };

    $result = gm_reset_game_files($db, $config, $gameId, $mappedProgress);
    gm_emit($progress, 'database_cleanup', 0, max(1, count($unverifiedRows) + $pakCount), $innerEnd + 1, 'Removing retained PAK records and game-associated staging rows…');
    ...
    try {
        ...
        foreach (array_chunk(array_column($unverifiedRows, 'id'), 100) as $chunk) {
            ...
            $stmt = $db->prepare('DELETE FROM ue_files WHERE id IN (' . implode(',', array_fill(0, count($chunk), '?')) . ')');
            $stmt->execute($chunk);
    ...
    gm_emit($progress, 'database_cleanup', count($unverifiedRows) + $pakCount, max(1, count($unverifiedRows) + $pakCount), $endPercent, 'All game file, PAK, and staging records have been removed.');
...
function gm_lifecycle_delete_game(PDO $db, array $config, int $gameId, ?callable $progress = null): array
{
    $game = catalog_one(
        $db,
        'SELECT g.id,g.name,g.slug,(SELECT COUNT(*) FROM ue_sources s WHERE s.game_id=g.id) source_count FROM ue_games g WHERE g.id=?',
        [$gameId]
    );
    if (!$game) {
    ...
    gm_emit($progress, 'delete_game', 1, 1, 76, 'Game configuration deleted; foreign-key projection rows were removed.');
    $optimise = gm_lifecycle_optimise_tables($db, gm_lifecycle_optimise_table_list(true), $progress, 78, 99);
