...
/**
 * Identifies existing files whose dependency resolution can change after a new
 * package is added. A new package only affects package-only imports for its
 * package name and object imports for paths exported by that package.
 */
final class CatalogAffectedDependencyRefreshService
{
    ...
    public static function findAffectedFileIds(PDO $db, int $gameId, int $newFileId, string $packageName): array
    {
        ...
        $exportPaths = catalog_all($db, 'SELECT full_path FROM ue_exports WHERE file_id=?', [$newFileId]);
        $paths = array_values(array_unique(array_map(static fn(array $row): string => (string)$row['full_path'], $exportPaths), SORT_STRING));
