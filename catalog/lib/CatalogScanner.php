...
function scanner_package_name_from_reader(string $fallbackPackageName, string $readerEngine, array $names, array $header): string
{
    ...
    foreach ($names as $name) {
        ...
        if ($text !== '' && str_starts_with($text, '/')) $candidates[] = $text;
    }
    ...
    foreach (array_values(array_unique($candidates)) as $candidate) {
        ...
        if ($fallbackLeaf === '' || $leaf === $fallbackLeaf) return $candidate;
    }
...
function scanner_rebuild_dependencies(PDO $db, array $config, int $fileId, ?callable $progress = null, int $startPercent = 0, int $endPercent = 100, string $prefix = 'Rebuilding dependencies'): void
{
    ...
    if (!$file) { scanner_emit_percent($progress, 'dependencies', $endPercent, $prefix . ': skipped missing file'); return; }
    ...
    foreach ($imports as $i => $imp) {
        ...
        $done = $i + 1;
        if (($done % 10) === 0 || $done === $total) scanner_emit_percent($progress, 'dependencies', scanner_range_percent($startPercent, $endPercent, $done, $total), $prefix . ': import ' . $done . '/' . $total);
    }
}
...
function scanner_rebuild_affected_dependencies(PDO $db, array $config, int $newFileId, ?callable $progress = null, int $startPercent = 56, int $endPercent = 99): void
{
    ...
    if (!$file) { scanner_emit_percent($progress, 'dependencies', $endPercent, 'Refreshing affected dependency links: imported file missing'); return; }
    ...
    if ($total === 0) { scanner_emit_percent($progress, 'dependencies', $endPercent, 'Refreshing affected dependency links: no existing files affected'); return; }
...
function scanner_rebuild_affected_dependencies_for_package(PDO $db, array $config, int $gameId, string $packageName, ?callable $progress = null, int $startPercent = 56, int $endPercent = 99): void
{
    ...
    $total = count($files);
    if ($total === 0) { scanner_emit_percent($progress, 'dependencies', $endPercent, 'Refreshing alias dependency links: no existing files affected'); return; }
...
function scanner_scan_uploaded_file(PDO $db, array $config, int $gameId, string $tmp, string $originalName, ?int $userId, bool $strictProfile = true, ?callable $progress = null, bool $allowProfileOverride = false): array
{
    ...
    if ($extensionOutsideProfile && !$allowProfileOverride) throw new RuntimeException('Extension not allowed by assigned profile: ' . $ext . '. Allowed: ' . implode(', ', $profileExtensions));
    $size = filesize($tmp) ?: 0;
    if ($size <= 0 || $size > (int)$config['max_upload_bytes']) throw new RuntimeException('Bad file size: ' . catalog_bytes((int)$size));
    ...
    if ($strictProfile && empty($classification['ok_for_selected_game'])) {
        $suggested = [];
        foreach ($classification['suggested_games'] as $s) $suggested[] = $s['game_name'] . ' (' . $s['engine_key'] . ')';
    ...
    if ((!$strictProfile || $allowProfileOverride) && in_array($detectedEngine, ['UE1','UE2','UE3','UE4','UE5'], true)) $readerEngine = $detectedEngine;
    if ($readerEngine === '') $readerEngine = $profileEngine;
    ...
    if ($packageGuid !== '') $duplicate = catalog_one($db, 'SELECT id, original_name, package_name, package_guid, file_size, md5 FROM ue_files WHERE game_id=? AND package_guid=? AND md5=?', [$gameId, $packageGuid, $md5]);
    else $duplicate = catalog_one($db, 'SELECT id, original_name, package_name, package_guid, file_size, md5 FROM ue_files WHERE game_id=? AND md5=? AND (package_guid IS NULL OR package_guid="")', [$gameId, $md5]);
    if ($duplicate) {
        ...
        if (strcasecmp($duplicatePackageName, $packageName) === 0 || catalog_package_alias_exists($db, $duplicateFileId, $gameId, $packageName)) { scanner_emit_percent($progress, 'done', 100, 'Duplicate in selected game'); return ['duplicate', $duplicateFileId, 'Duplicate in selected game', $classification, $meta]; }
        catalog_package_alias_add($db, $duplicateFileId, $gameId, $packageName, $originalName, $packageGuid, $md5, (int)$size);
        $refreshWarning = '';
        try { scanner_rebuild_affected_dependencies_for_package($db, $config, $gameId, $packageName, $progress, 56, 99); }
        catch (Throwable $refreshError) { error_log('[UnrealDB dependency refresh] alias_package=' . $packageName . ' file_id=' . $duplicateFileId . ' error=' . $refreshError->getMessage()); $refreshWarning = '; dependency refresh warning logged for maintenance'; }
        scanner_emit_percent($progress, 'done', 100, 'Alias package added for existing file identity');
        $meta['alias_package_name'] = $packageName; $meta['alias_added'] = true;
    ...
    $scanNotesAll = array_merge($scanNotes, ['Profile engine=' . $profileEngine . '; package reader=' . $readerEngine . $parserNote . '; package=' . $packageName . '; compatibility=' . ($classification['compatibility_status'] ?? 'native') . '; detection=' . $classification['confidence'] . $cleanNote . '; ' . implode(' ', $classification['notes'])]);
    if ($extensionOutsideProfile) $scanNotesAll[] = 'Administrator override: extension .' . $ext . ' is outside the assigned profile extension list.';
    if (!$strictProfile && ($detectedEngine !== '' && $detectedEngine !== $profileEngine)) $scanNotesAll[] = 'Administrator compatibility override: catalogued under ' . $profileEngine . ' game using detected ' . $detectedEngine . ' reader.';
    ...
    if (is_file($dest)) { if (is_file($tmp) && !@unlink($tmp)) throw new RuntimeException('Could not discard duplicate physical upload'); }
    elseif (!rename($tmp, $dest)) throw new RuntimeException('Could not store upload');
    else $storedFileCreated = true;
    ...
    $writtenRows = 0;
    $progressDb = static function (string $message, int $rowsDone = 1) use ($progress, &$writtenRows, $totalRows): void { $writtenRows = min($totalRows, $writtenRows + max(1, $rowsDone)); scanner_emit_percent($progress, 'database', scanner_range_percent(23, 35, $writtenRows, $totalRows), $message); };
    ...
    try {
        ...
        foreach ($names as $i => $name) { $stmt->execute([$fileId, $i, (string)($name['name'] ?? $name['text'] ?? ''), isset($name['flags']) ? (int)$name['flags'] : null]); $done = $i + 1; if (($done % 10) === 0 || $done === $nameCount) $progressDb('Writing names table ' . $done . '/' . $nameCount, 10); }
        $cache = [];
        ...
        foreach ($imports as $i => $imp) {
            ...
            $done = $i + 1; if (($done % 10) === 0 || $done === $importCount) $progressDb('Writing imports table ' . $done . '/' . $importCount, 10);
        }
        ...
        foreach ($exports as $i => $exp) {
            ...
            $done = $i + 1; if (($done % 10) === 0 || $done === $exportCount) $progressDb('Writing exports table ' . $done . '/' . $exportCount, 10);
        }
        ...
        if ($storedFileCreated && is_file($dest)) @unlink($dest);
        throw $e;
    }

    $refreshWarning = '';
    try { scanner_rebuild_affected_dependencies($db, $config, $fileId, $progress, 56, 99); }
    catch (Throwable $refreshError) { error_log('[UnrealDB dependency refresh] imported_file_id=' . $fileId . ' error=' . $refreshError->getMessage()); $refreshWarning = '; dependency refresh warning logged for maintenance'; }
