...
/**
 * Central, bounded error capture for browser-facing catalog requests.
 *
 * The logger never throws back into the request that it is observing. When the
 * database or migration is unavailable, the normal PHP/Apache error log remains
...
function catalog_system_error_register(): void
{
    ...
    $previousErrorHandler = set_error_handler(
        static function (int $type, string $message, string $file, int $line) use (&$previousErrorHandler): bool {
            ...
            if (is_callable($previousErrorHandler)) {
                return (bool)$previousErrorHandler($type, $message, $file, $line);
            }
            return false;
        }
    );
    ...
    $previousExceptionHandler = set_exception_handler(
        static function (Throwable $error) use (&$previousExceptionHandler): void {
            ...
            error_log('[UnrealDB][' . catalog_system_error_request_id() . '] uncaught '
            ...
            echo 'The request could not be completed. Reference: ' . catalog_system_error_request_id();
        }
    );
    ...
    catalog_system_error_register_browser_asset();
}

function catalog_system_error_register_browser_asset(): void
{
    if (PHP_SAPI === 'cli' || !empty($GLOBALS['catalog_system_error_browser_asset_registered'])) {
        return;
    }
    $GLOBALS['catalog_system_error_browser_asset_registered'] = true;

    ob_start(static function (string $html): string {
        if (!str_contains($html, '</body>') || str_contains($html, 'catalog-system-errors.js')) {
            return $html;
        }
        ...
        $asset = dirname(__DIR__) . '/assets/catalog-system-errors.js';
        $version = is_file($asset) ? (string)(filemtime($asset) ?: 1) : '1';
        $tag = '<script src="' . htmlspecialchars(
            $root . 'assets/catalog-system-errors.js?v=' . rawurlencode($version),
...
function catalog_system_error_record_http(
    string $code,
    string $message,
    int $status,
    array $context = []
...
function catalog_system_error_normalize(array $data): array
{
    ...
    $httpStatus = max(0, min(599, (int)($data['http_status'] ?? http_response_code())));
    ...
    $contextJson = catalog_system_error_text($contextJson, 16000);
