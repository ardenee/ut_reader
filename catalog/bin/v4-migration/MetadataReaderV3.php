<?php
/**
 * Offline format-3 reader used only by the v3 -> v4 migration.
 * Production/runtime metadata code must never depend on this class.
 * Future v5+ cutovers should add their prior-format reader under the matching
 * migration directory rather than adding compatibility to runtime readers.
 */
declare(strict_types=1);

namespace UnrealDb\Catalog\MigrationV4;

use JsonException;
use PDO;
use RuntimeException;

/** Reads only the required blocks from a version-3 metadata container. */
final class MetadataReaderV3
{
    ...
    public function verify(int $fileId): array
    {
        ...
        $verified = MetadataReaderV3::verifyV3File(
            (string)$context['path'],
            $fileId,
    ...
    private function manifest(int $fileId, bool $cache = true): array
    {
        ...
        $path = self::path($this->storageRoot, (int)$row['game_id'], $fileId);
    ...
    public static function verifyV3File(
        string $path,
        int $expectedFileId,
        ?string $expectedPayloadSha256 = null,
        int $expectedFormatVersion = 3
    ): array {
        clearstatcache(true, $path);
        $size = @filesize($path);
        if ($size === false || $size < 20) {
            throw new RuntimeException('V3 metadata container is missing or too small: ' . $path);
        }
        $stream = @fopen($path, 'rb');
        if (!is_resource($stream)) {
            throw new RuntimeException('Could not open v3 metadata container: ' . $path);
        }
        $hash = hash_init('sha256');
        try {
            $headerBytes = self::readExactlyStatic($stream, 20);
            hash_update($hash, $headerBytes);
            $header = unpack('a8magic/vversion/vcodec/Vmanifest_length/Vreserved', $headerBytes);
            if ($expectedFormatVersion !== 3
                || !is_array($header)
                || (string)$header['magic'] !== "UEDBM3\0\0"
                || (int)$header['version'] !== 3
                || (int)$header['codec'] !== 2) {
                throw new RuntimeException('Metadata container is not format 3.');
            }
            $manifestLength = (int)$header['manifest_length'];
            if ($manifestLength < 2 || 20 + $manifestLength > (int)$size) {
                throw new RuntimeException('V3 metadata manifest length is invalid.');
            }
            $manifestBytes = self::readExactlyStatic($stream, $manifestLength);
            ...
            if (!is_array($manifest) || (int)($manifest['file']['id'] ?? 0) !== $expectedFileId) {
                throw new RuntimeException('V3 metadata manifest identity mismatch.');
            }
            $offset = 0;
            $blocks = 0;
            foreach ((array)($manifest['sections'] ?? []) as $sectionBlocks) {
                foreach ((array)$sectionBlocks as $block) {
                    if (!is_array($block)) {
                        throw new RuntimeException('V3 metadata block descriptor is invalid.');
                    }
                    $blockOffset = (int)($block['offset'] ?? -1);
                    $length = (int)($block['compressed_length'] ?? 0);
                    if ($blockOffset !== $offset || $length < 1) {
                        throw new RuntimeException('V3 metadata block ordering is invalid.');
                    }
                    $compressed = self::readExactlyStatic($stream, $length);
                    hash_update($hash, $compressed);
                    if (!hash_equals((string)($block['sha256'] ?? ''), hash('sha256', $compressed))) {
                        throw new RuntimeException('V3 metadata block checksum mismatch.');
                    }
                    $json = gzdecode($compressed);
                    if (!is_string($json) || strlen($json) !== (int)($block['uncompressed_length'] ?? -1)) {
                        throw new RuntimeException('V3 metadata block decompression failed.');
                    }
                    $offset += $length;
                    $blocks++;
                }
    ...
    private static function readExactlyStatic($stream, int $length): string
    {
        $buffer = '';
        while (strlen($buffer) < $length && !feof($stream)) {
            $chunk = fread($stream, $length - strlen($buffer));
            if ($chunk === false) {
                throw new RuntimeException('Could not read v3 metadata container.');
            }
            $buffer .= $chunk;
        }
        if (strlen($buffer) !== $length) {
            throw new RuntimeException('V3 metadata container ended unexpectedly.');
        }
        return $buffer;
    }
