...
try {
    ...
    $totalOverflowTerms = compact_readiness_scalar(
        $db,
        'SELECT COUNT(*) FROM ue_terms WHERE is_overflow=1'
    );
    $incompleteOverflowTerms = compact_readiness_scalar(
        $db,
    ...
    if ($incompleteOverflowTerms !== 0) {
        $dataBlockers[] = $incompleteOverflowTerms
            . ' compact overflow term(s) are still truncated or fail their stored hash.';
    }
