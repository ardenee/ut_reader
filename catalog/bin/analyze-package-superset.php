...
<?php
/**
 * Query catalog-wide current-format package object coverage without mutating catalog state.
 *
 * Example:
 *   php catalog/bin/analyze-package-superset.php --game-id=2 --package=Foo
 */
...
if ($gameId < 1 || $packageName === '') {
    fwrite(STDERR, "Usage: php catalog/bin/analyze-package-superset.php --game-id=ID --package=NAME [--max-paths=1000] [--max-providers=250]\n");
    exit(2);
}
