 ...
 * Purpose: Rebuilds current metadata projections from authoritative current-format containers.
...
try {
    ...
    $statement = $db->prepare(
        ...
        . 'WHERE m.format_version=' . \UnrealDb\Catalog\Infrastructure\Metadata\BlockedCompressedMetadataContainer::FORMAT_VERSION . ' AND f.scan_status="verified" AND f.id>? '
    ...
    if ($files === []) {
        fwrite(STDOUT, "No matching current-format metadata files require projection rebuilding.\n");
        exit(0);
    }
