 ...
 * Purpose: Provides the command-line utility for rebuild compact file dependencies.
...
try {
    ...
    if ($arguments['file_id'] > 0) {
        $statement = $db->prepare(
            ...
            . 'FROM ue_files f JOIN ue_file_metadata m ON m.file_id=f.id AND m.format_version=' . \UnrealDb\Catalog\Infrastructure\Metadata\BlockedCompressedMetadataContainer::FORMAT_VERSION . ' '
    ...
    if (!is_array($file)) {
        throw new RuntimeException('No matching verified current-format file was found.');
    }
