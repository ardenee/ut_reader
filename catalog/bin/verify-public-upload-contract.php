...
$check(
    ...
    str_contains($page, 'public-upload-export-log')
        ...
        && str_contains($client, 'function diagnosticLogText(maxBytes)')
        && str_contains($client, 'MAX_DIAGNOSTIC_PROBLEM_LINES')
        && str_contains($client, 'diagnosticProblemLines')
        && str_contains($client, "['rejected', 'failed', 'stopped'].includes(diagnosticEntry.status)")
        && str_contains($client, "'Problem/rejection log:'")
        && str_contains($client, "new Blob([diagnosticLogText(0)]")
        && str_contains($client, "hiddenField(feedbackForm, 'prepare_diagnostic_log', '1')")
        && str_contains($client, "hiddenField(feedbackForm, 'diagnostic_log', diagnosticLogText(MAX_FEEDBACK_LOG_BYTES))")
