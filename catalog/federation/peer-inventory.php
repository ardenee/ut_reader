...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'refresh') {
            ...
            fed_log($db, $peerId, null, 'INFO', 'PARENT_PULL_QUEUE', 'Queued ' . $queued . ' child file(s) from the simplified inventory page.');
            $_SESSION['fed_peer_inventory_flash'] = 'Queued ' . $queued . ' file(s) from this child. Existing and protected files were skipped.';
        } else {
    ...
    $summary = catalog_one(
        $db,
        'SELECT
            SUM(CASE WHEN ' . $absenceSql . ' AND ' . $parentDependencySql . ' AND NOT (' . $baseGameSql . ') THEN 1 ELSE 0 END) parent_dependency_needs,
            SUM(CASE WHEN ' . $absenceSql . ' AND NOT (' . $baseGameSql . ') THEN 1 ELSE 0 END) parent_needs
         FROM ue_federation_peer_files pf
         WHERE pf.peer_id=?',
        [$peerId]
    ) ?: [];
    ...
    echo '<div><a class="button" href="' . catalog_h(pi_url($peerId, 'parent_dependency')) . '">Parent Dependency Needs (' . $parentDependencyCount . ')</a><p class="muted small">Files this child has that the parent needs to complete known missing dependencies.</p></div>';
    echo '<div><a class="button" href="' . catalog_h(pi_url($peerId, 'parent_needs')) . '">Parent Needs (' . $parentNeedsCount . ')</a><p class="muted small">All transferable files this child has that the parent does not have, including dependency needs.</p></div>';
    echo '<div><a class="button" href="' . catalog_h(pi_url($peerId, 'child_dependency')) . '">Child Dependency Needs (' . $childDependencyCount . ')</a><p class="muted small">Files this parent has that the child requested for missing dependencies. The child cannot download anything else.</p></div>';
    echo '</div></div>';
    ...
    if ($filter === 'child_dependency') {
        ...
        echo '<div class="card"><h2>Child Dependency Needs</h2><p>These are dependency requests from <strong>' . catalog_h((string)$peer['site_name']) . '</strong> for which this parent currently has a transferable file. Review the request; the child downloads only after approval.</p>';
        if (!$rows) {
            echo '<p class="muted">No child dependency needs currently match a transferable parent file.</p>';
        } else {
        ...
        $filterSql = $filter === 'parent_dependency'
            ? ' AND ' . $absenceSql . ' AND ' . $parentDependencySql . ' AND NOT (' . $baseGameSql . ')'
            : ' AND ' . $absenceSql . ' AND NOT (' . $baseGameSql . ')';
        ...
        $description = $filter === 'parent_dependency'
            ? 'Files held by this child that directly satisfy missing dependencies on the parent.'
            : 'Every transferable child file absent from the parent. Rows marked dependency are the subset required by current parent dependencies.';
        ...
        if (!$rows) {
            ...
            foreach ($rows as $row) {
                ...
                echo '<tr><td><input type="checkbox" data-check-group="parent-files" name="peer_file_ids[]" value="' . (int)$row['id'] . '"></td><td><span class="pill ' . ($dependencyCount > 0 ? 'amber' : '') . '">' . ($dependencyCount > 0 ? 'dependency' : 'not held') . '</span></td><td>' . catalog_h((string)($row['display_game'] ?? '')) . '<div class="muted small">' . catalog_h((string)$row['remote_engine_key']) . '</div></td><td>' . $dependencyCount . '</td><td class="mono">' . catalog_h((string)$row['package_name']) . '</td><td>' . catalog_h((string)$row['original_name']) . '</td><td class="nowrap">' . (int)$row['import_count'] . ' / ' . (int)$row['export_count'] . '</td><td>' . pi_identity_html($row) . '</td><td class="nowrap">' . catalog_h(catalog_bytes((int)$row['file_size'])) . '</td></tr>';
            }
