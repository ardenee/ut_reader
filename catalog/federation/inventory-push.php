...
function build_inventory_payload(PDO $db): array
{
    ...
    return ['site' => $identity, 'generated_at' => date('c'), 'file_count' => count($out), 'files' => $out];
}
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!$peer) throw new RuntimeException('Active parent peer not found.');
        $secret = (string)($peer['shared_secret_plain'] ?? '');
        if ($secret === '') throw new RuntimeException('Selected parent peer has no stored API secret.');

        $url = rtrim((string)$peer['site_url'], '/') . '/api/federation/inventory-push.php';
        $result = fed_http_post_signed($url, (string)fed_setting($db, 'site_id', ''), $secret, build_inventory_payload($db));
    ...
    if (!catalog_require_admin_page('Push Inventory')) exit;
    ...
    echo '<div class="card"><h2>Inventory summary</h2><p>Verified local files ready to report: <strong>' . $count . '</strong></p></div><div class="card"><h2>Run push</h2>';
    if (!$parents) {
        ...
        foreach ($parents as $parent) echo '<option value="' . (int)$parent['id'] . '">' . catalog_h($parent['site_name'] . ' - ' . $parent['site_url']) . '</option>';
        echo '</select></label></p><button>Push inventory to parent</button></form>';
    }
    echo '</div>';
    catalog_foot();
} catch (Throwable $e) {
    if (!headers_sent()) catalog_head('Push inventory error');
