 ...
 * Purpose: Verifies upload bucket worker queue behavior as an automated regression/contract test.
...
bucket_worker_queue_expect(
    str_contains($uploadClient, 'upload_ids: [item.uploadId]')
        && str_contains($uploadClient, 'start_worker: false')
        && str_contains($uploadClient, "upload_ids: [],")
        && str_contains($uploadClient, 'start_worker: true'),
    'Browser does not finalise files individually before one explicit worker start.'
);
bucket_worker_queue_expect(str_contains($batchEndpoint, 'CatalogDetachedWorker'), 'File finalisation cannot start the processing worker.');
bucket_worker_queue_expect(str_contains($batchEndpoint, 'if ($prepareQueue || $startWorker)'), 'Intermediate file finalisation still repeats worker inspection.');
...
bucket_worker_queue_expect(str_contains($queue, "':bucket-processing'"), 'New Upload Bucket work does not use the dedicated processing queue.');
...
echo "File-by-file Upload Bucket worker queue contract tests passed.\n";
