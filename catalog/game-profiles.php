...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($gameId <= 0 || $engine === '') {
            throw new RuntimeException('Game and engine are required.');
        }
    ...
    echo '<div class="card hero"><h1>Game Profiles</h1><p class="muted">Game-specific scanner rules. These make it easier to add more Unreal Engine games later, including UE5 titles, without hardcoding everything into the scanner.</p>';
    catalog_page_links(['Setup' => 'setup.php', 'Profiled Upload' => 'profiled-upload.php', 'Library' => 'library.php']);
    echo '</div>';

    echo '<div class="card"><h2>Profiles</h2><table><tr><th>Game</th><th>DB Engine</th><th>Profile Engine</th><th>Extensions</th><th>Version</th><th>Policy</th><th>Active</th><th>Edit</th></tr>';
    foreach ($games as $game) {
        ...
        echo '<tr><td>' . catalog_h($game['name']) . '</td><td class="mono">' . catalog_h($game['engine_key']) . '</td><td class="mono">' . catalog_h($game['profile_engine'] ?? '') . '</td><td class="mono">' . catalog_h(is_array($exts) ? implode(', ', $exts) : '') . '</td><td class="mono">' . catalog_h($range) . '</td><td>' . catalog_h($game['confidence_policy'] ?? '') . '</td><td>' . ((int)($game['is_active'] ?? 0) ? 'yes' : 'no') . '</td><td><a class="button" href="game-profiles.php?game_id=' . (int)$game['id'] . '">edit</a></td></tr>';
    }
    echo '</table></div>';
    ...
    if ($edit) {
        ...
        echo '<tr><th>Engine key</th><td><input name="engine_key" value="' . catalog_h($edit['profile_engine'] ?? $edit['engine_key']) . '" style="width:120px"> <span class="muted">Examples: UE1, UE2, UE3, UE4, UE5</span></td></tr>';
