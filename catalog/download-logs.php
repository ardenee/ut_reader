...
try {
    ...
    if ($available) {
        ...
        if ($view === 'downloads') {
            ...
            if ($search !== '') {
                ...
                $where[] = '(a.download_name LIKE ? OR a.package_format LIKE ? OR a.user_agent LIKE ? OR a.error_message LIKE ?)';
            ...
            $statement = $db->prepare(
                ...
                . $whereSql
                . ' ORDER BY a.started_at DESC,a.id DESC LIMIT ' . $perPage . ' OFFSET ' . $offset
            );
        } else {
            if ($search !== '') {
                $like = '%' . str_replace(['\\', '%', '_'], ['\\\\', '\\%', '\\_'], $search) . '%';
                $where[] = '(a.package_name LIKE ? OR a.package_format LIKE ? OR a.user_agent LIKE ? OR a.error_message LIKE ? OR a.artifact_name LIKE ?)';
            ...
            $statement = $db->prepare(
                ...
                . 'LEFT JOIN ue_files f ON f.id=a.file_id '
                . $whereSql
                . ' ORDER BY a.queued_at DESC,a.id DESC LIMIT ' . $perPage . ' OFFSET ' . $offset
            );
        }
    ...
    echo '<style>'
        ...
        . '.download-log-table{min-width:1450px}'
    ...
    catalog_page_header(
        'Download Logs',
        'Administrator reporting records for generated package requests and actual individual/generated-package transfers. IP addresses are stored compactly and displayed here in normal IPv4 or IPv6 form.',
        [
    ...
    echo '</select></label>'
        ...
        . '<label class="search">Search <input type="search" name="q" value="' . catalog_h($search) . '" placeholder="File, package, format, error or user agent"></label>'
    ...
    if (!$available || !$rows) {
        ...
        echo '<div class="table-wrap"><table class="download-log-table"><thead><tr>'
            . '<th>Started</th><th>Status</th><th>Type</th><th>File / package</th><th>Game</th><th>IP</th>'
            . '<th>Transferred</th><th>Range / HTTP</th><th>Job</th><th>User agent / error</th>'
            . '</tr></thead><tbody>';
        foreach ($rows as $row) {
            ...
            echo '<td>' . catalog_h(str_replace('_', ' ', (string)$row['download_type'])) . '</td>';
            ...
            echo '<td class="mono small">' . (isset($row['range_start']) ? (int)$row['range_start'] : 0) . '–' . (isset($row['range_end']) ? (int)$row['range_end'] : 0)
                . '<br>HTTP ' . (int)$row['http_status'] . '</td>';
        ...
        echo '<div class="table-wrap"><table class="download-log-table"><thead><tr>'
            . '<th>Queued</th><th>Status</th><th>Package</th><th>Version</th><th>Format</th><th>Game / file</th>'
            . '<th>IP</th><th>Artifact</th><th>Job</th><th>User agent / error</th>'
            . '</tr></thead><tbody>';
        foreach ($rows as $row) {
            $rowStatus = strtolower((string)$row['status']);
