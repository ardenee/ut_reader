...
`catalog/install.sql` is the canonical **squashed baseline** for a new empty MySQL 8+ or compatible MariaDB database. The current consolidated baseline contains schema and seed-data changes through version `202609240002`.
...
A fresh database loads the consolidated baseline and is current immediately unless migration files newer than baseline `202609240002` are present:
...
At the current baseline there are no active post-baseline migrations.

## Current incremental migrations

There are currently no active incremental migrations newer than baseline `202609240002`. Future schema changes must use a version greater than that boundary and remain incremental until the next deliberate consolidation.
