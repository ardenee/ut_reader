 ...
 * Purpose: Supports the standalone `UE3` parser/viewer workflow for UE3.
...
function resolve_package_path(string $fileParam, string $uploadDir, array $uploadedFiles): string
{
    ...
    foreach (['testde.ut3', 'test.ut3', 'test.upk', 'test.utx'] as $defaultName) {
...
function name_index_link(int $idx): string
{
    ...
    return '<a class="name-tag name-link" href="#' . h(name_ref_target_id($idx)) . '">#' . h($idx) . '</a>';
}
...
function read_fstring32_raw(string $bytes, int $offset): array
{
    if ($offset < 0 || $offset + 4 > strlen($bytes)) {
...
function build_ue3_raw_header_fields(string $packagePath): array
{
    ...
    if ($version >= 249) {
        $headerSize = read_le_u32($bytes, $pos);
        add_raw_header_field($fields, $bytes, $pos, 4, 'headerSize', 'uint32', $headerSize);
        $pos += 4;
    }

    if ($version >= 269) {
        [$folderName, $folderSize, $folderNote] = read_fstring32_raw($bytes, $pos);
    ...
    $nameCount = read_le_u32($bytes, $pos);
    add_raw_header_field($fields, $bytes, $pos, 4, 'nameCount', 'uint32', $nameCount);
    $pos += 4;

    $nameOffset = read_le_u32($bytes, $pos);
    add_raw_header_field($fields, $bytes, $pos, 4, 'nameOffset', 'uint32', $nameOffset);
    $pos += 4;

    $exportCount = read_le_u32($bytes, $pos);
    add_raw_header_field($fields, $bytes, $pos, 4, 'exportCount', 'uint32', $exportCount);
    $pos += 4;

    $exportOffset = read_le_u32($bytes, $pos);
    add_raw_header_field($fields, $bytes, $pos, 4, 'exportOffset', 'uint32', $exportOffset);
    $pos += 4;

    $importCount = read_le_u32($bytes, $pos);
    add_raw_header_field($fields, $bytes, $pos, 4, 'importCount', 'uint32', $importCount);
    $pos += 4;

    $importOffset = read_le_u32($bytes, $pos);
    add_raw_header_field($fields, $bytes, $pos, 4, 'importOffset', 'uint32', $importOffset);
    $pos += 4;

    $dependsOffset = 0;
    if ($version >= 415) {
        $dependsOffset = read_le_u32($bytes, $pos);
        add_raw_header_field($fields, $bytes, $pos, 4, 'dependsOffset', 'uint32', $dependsOffset);
        $pos += 4;
    }

    $importExportGuidsOffset = 0;
    if ($version >= 623) {
        $importExportGuidsOffset = read_le_u32($bytes, $pos);
        add_raw_header_field($fields, $bytes, $pos, 4, 'importExportGuidsOffset', 'uint32', $importExportGuidsOffset);
        $pos += 4;

        $importGuidsCount = read_le_u32($bytes, $pos);
        add_raw_header_field($fields, $bytes, $pos, 4, 'importGuidsCount', 'uint32', $importGuidsCount);
        $pos += 4;

        $exportGuidsCount = read_le_u32($bytes, $pos);
        add_raw_header_field($fields, $bytes, $pos, 4, 'exportGuidsCount', 'uint32', $exportGuidsCount);
        $pos += 4;
    }

    $thumbnailTableOffset = 0;
    if ($version >= 584) {
        $thumbnailTableOffset = read_le_u32($bytes, $pos);
        add_raw_header_field($fields, $bytes, $pos, 4, 'thumbnailTableOffset', 'uint32', $thumbnailTableOffset);
        $pos += 4;
    }
    ...
    $generationCount = read_le_u32($bytes, $pos);
    add_raw_header_field($fields, $bytes, $pos, 4, 'generationCount', 'uint32', $generationCount);
    $pos += 4;

    for ($i = 0; $i < $generationCount && $i < 1024 && $pos + 8 <= strlen($bytes); $i++) {
        $offset = $pos;
        $e = read_le_u32($bytes, $pos);
        $n = read_le_u32($bytes, $pos + 4);
        $net = 0;
        $size = 8;
        $value = 'exportCount=' . $e . ', nameCount=' . $n;

        if ($version >= 322 && $pos + 12 <= strlen($bytes)) {
            $net = read_le_u32($bytes, $pos + 8);
            $size = 12;
            $value .= ', netObjectCount=' . $net;
        }

        add_raw_header_field($fields, $bytes, $offset, $size, 'generation[' . $i . ']', 'FGenerationInfo', $value);
        $pos += $size;
    }

    if ($version >= 245) {
        $engineVersion = read_le_u32($bytes, $pos);
        add_raw_header_field($fields, $bytes, $pos, 4, 'engineVersion', 'uint32', $engineVersion);
        $pos += 4;
    }

    if ($version >= 277) {
        $cookerVersion = read_le_u32($bytes, $pos);
        add_raw_header_field($fields, $bytes, $pos, 4, 'cookerVersion', 'uint32', $cookerVersion);
        $pos += 4;
    }

    $compressionFlags = 0;
    if ($version >= 334) {
        $compressionFlags = read_le_u32($bytes, $pos);
        add_raw_header_field($fields, $bytes, $pos, 4, 'compressionFlags', 'uint32', $compressionFlags);
        $pos += 4;
    }

    if ($compressionFlags !== 0 && $pos + 4 <= strlen($bytes)) {
        $chunkCount = read_le_u32($bytes, $pos);
        add_raw_header_field($fields, $bytes, $pos, 4, 'compressedChunkCount', 'uint32', $chunkCount);
        $pos += 4;

        for ($i = 0; $i < $chunkCount && $i < 1024 && $pos + 16 <= strlen($bytes); $i++) {
            $uOff = read_le_u32($bytes, $pos);
            $uLen = read_le_u32($bytes, $pos + 4);
            $cOff = read_le_u32($bytes, $pos + 8);
            $cLen = read_le_u32($bytes, $pos + 12);
            add_raw_header_field($fields, $bytes, $pos, 16, 'compressedChunk[' . $i . ']', 'FCompressedChunk', 'uOff=' . $uOff . ', uLen=' . $uLen . ', cOff=' . $cOff . ', cLen=' . $cLen);
            $pos += 16;
        }
    }

    if ($version >= 482 && $pos + 4 <= strlen($bytes)) {
        $firstTableOffset = first_positive_offset([$nameOffset, $importOffset, $exportOffset, $dependsOffset, $thumbnailTableOffset, $importExportGuidsOffset]);
        if ($firstTableOffset === 0 || $pos + 4 <= $firstTableOffset) {
            $unknown = read_le_u32($bytes, $pos);
            add_raw_header_field($fields, $bytes, $pos, 4, 'unknownUE3HeaderField_v482', 'uint32', $unknown);
            $pos += 4;
        }
    }

    $headerEnd = first_positive_offset([$nameOffset, $importOffset, $exportOffset, $dependsOffset, $thumbnailTableOffset, $importExportGuidsOffset]);
    add_unparsed_header_bytes($fields, $bytes, $pos, $headerEnd, 'Bytes between decoded UE3 header fields and the first package table.');
...
if ($filePath === '' || !file_exists($filePath)) {
    ...
    echo "UE3.php: no package file is available.\n";
    echo "Use UE3/upload.php, put a supported UE3 package into UE3/uploads/, or keep testde.ut3 beside UE3.php.\n";
    exit;
}
...
$rawHeaderFields = build_ue3_raw_header_fields($filePath);
...
function renderPropsTree(array $props): void
{
    ...
    <div class="props-tree">
        <?php foreach ($props as $p): ?>
            ...
            $extras = $p;
            unset($extras['name'], $extras['type'], $extras['typeName'], $extras['value'], $extras['rawHex']);
            ?>
            <details class="prop-node">
                <summary>
                    <span class="ico prop">▤</span><span class="prop-name"><?= h($name) ?></span>
...
function renderImportNode(UnrealPackageReader $pkg, array $imports, array $importsByOuter, int $idx, bool $includeChildren, string $idPrefix = ''): void
{
    ...
    <details id="<?= h($anchor) ?>" class="tree-node" data-filter-row>
        <summary><span class="ico package">▣</span><span class="name"><?= name_link_html($pkg, $objectIdx, $object) ?></span></summary>
        <div class="tree-lines">
            <div>Object:<?= name_link_html($pkg, $objectIdx, $object) ?><a class="ref-tag ref-link" href="#<?= h(object_ref_target_id($ref)) ?>">(<?= h($ref) ?>)</a></div>
            ...
            <?php foreach ($children as $childIdx): ?><?php renderImportNode($pkg, $imports, $importsByOuter, (int)$childIdx, true, $idPrefix); ?><?php endforeach; ?>
        </div>
    </details>
    <?php
}
...
function renderExportNode(UnrealPackageReader $pkg, array $exports, array $exportsByOuter, int $idx, bool $withProps = false, string $idPrefix = '', bool $includeChildren = true): void
{
    ...
    <details id="<?= h($anchor) ?>" class="tree-node" data-filter-row>
        <summary>
            <span class="ico export">≡</span><span class="name"><?= name_link_html($pkg, $objectIdx, $object) ?></span>
        ...
        <div class="tree-lines">
            ...
            <?php if (!empty($ex['archetype'])): ?><div class="extra-line">Archetype:<?= ref_value_html(object_ref_label($pkg, (int)$ex['archetype'])) ?></div><?php endif; ?>
            ...
            <?php if (!empty($ex['exportFlags'])): ?><div class="extra-line">Export Flags:<span class="mono"><?= h(hx_any($ex['exportFlags'])) ?></span></div><?php endif; ?>
            <?php if (!empty($ex['components'])): ?><div class="extra-line">Components:<span class="mono"><?= h(count($ex['components'])) ?></span></div><?php endif; ?>
            ...
            <?php if ($withProps && $props): ?>
                ...
                <div id="<?= h($propsId) ?>" class="props-block"><?php renderPropsTree($props); ?></div>
            <?php endif; ?>

            <?php foreach ($children as $childIdx): ?><?php renderExportNode($pkg, $exports, $exportsByOuter, (int)$childIdx, $withProps, $idPrefix, true); ?><?php endforeach; ?>
        </div>
    </details>
    <?php
}

function control_bar(string $scope): string
{
    return '<div class="tree-controls"><button type="button" class="btn" onclick="setTreeOpen(\'' . h($scope) . '\', true)">Expand all</button><button type="button" class="btn" onclick="setTreeOpen(\'' . h($scope) . '\', false)">Collapse all</button><button type="button" class="btn" onclick="copyTreeText(\'' . h($scope) . '\')">Copy text</button></div>';
}
?>
<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <title>UE3 Explorer — <?= h(basename($filePath)) ?></title>
    <style>
    ...
    <script>
        ...
        function setTreeOpen(id,open){const e=document.getElementById(id);if(!e)return;e.querySelectorAll('details.tree-node,details.prop-node').forEach(d=>d.open=open);}
        function copyTreeText(id){const e=document.getElementById(id);if(!e)return;setTreeOpen(id,true);const text=e.innerText.replace(/\n{3,}/g,'\n\n');navigator.clipboard&&navigator.clipboard.writeText(text);}
                ...
                <div class="file-open-bar">
                    <form method="get">
                        <select class="file-select" name="file">
                            ...
                            <?php endforeach; ?>
                            <?php foreach (['testde.ut3','test.ut3','test.upk','test.utx'] as $localDefault): ?>
            ...
            <section id="package-panel" class="panel active">
                <div class="package-grid">
                    <div class="field-grid">
                        ...
                        <div class="field-label">Engine Version</div><div class="field-value mono"><?= h($hdr['engineVersion'] ?? '') ?></div>
                        <div class="field-label">Cooker Version</div><div class="field-value mono"><?= h($hdr['cookerVersion'] ?? '') ?></div>
                        <div class="field-label">Signature</div><div class="field-value mono"><?= h(hx_any($hdr['signature'] ?? 0)) ?></div>
                    </div>
                    <div class="field-grid">
                        <div class="field-label">Flags</div><div class="field-value mono"><?= h(hx_any($hdr['pkgFlags'] ?? 0)) ?></div>
                        ...
                        <div class="field-label">Header Size</div><div class="field-value mono"><?= h($hdr['headerSize'] ?? '') ?></div>
                        <div class="field-label">Table Layouts</div><div class="field-value mono">N <?= h($hdr['nameTableLayout'] ?? '') ?> / E <?= h($hdr['exportTableLayout'] ?? '') ?></div>
                ...
                <h2>UE3 Offsets</h2>
                <table class="data"><tr><th>Name Count / Offset</th><td class="mono"><?= h($hdr['nameCount'] ?? '') ?> / <?= h($hdr['nameOffset'] ?? '') ?></td><th>Import Count / Offset</th><td class="mono"><?= h($hdr['importCount'] ?? '') ?> / <?= h($hdr['importOffset'] ?? '') ?></td><th>Export Count / Offset</th><td class="mono"><?= h($hdr['exportCount'] ?? '') ?> / <?= h($hdr['exportOffset'] ?? '') ?></td></tr><tr><th>Depends Offset</th><td class="mono"><?= h($hdr['dependsOffset'] ?? '') ?></td><th>Thumbnail Offset</th><td class="mono"><?= h($hdr['thumbnailTableOffset'] ?? '') ?></td><th>Import/Export GUIDs</th><td class="mono"><?= h($hdr['importExportGuidsOffset'] ?? '') ?></td></tr></table>
                <?php if (!empty($comp['isCompressed']) || !empty($comp['chunks'])): ?><h2>Compression</h2><table class="data"><tr><th>Compressed?</th><td class="mono"><?= !empty($comp['isCompressed']) ? 'Yes' : 'No' ?></td><th>Flags</th><td class="mono"><?= h(hx_any($comp['flags'] ?? 0)) ?></td><th>Logical size</th><td class="mono"><?= h($comp['logicalSize'] ?? 0) ?></td></tr><tr><th>Total compressed / uncompressed</th><td colspan="5" class="mono"><?= h($comp['totalCompressed'] ?? 0) ?> / <?= h($comp['totalUncompressed'] ?? 0) ?></td></tr></table><?php endif; ?>
                <?php if ($issues): ?><div class="warn"><strong>Validation / Notes</strong><ul><?php foreach ($issues as $w): ?><li class="mono raw"><?= h($w) ?></li><?php endforeach; ?></ul></div><?php endif; ?>
                <div class="to-top"><a href="#top">To top ↑</a></div>
            </section>

            <section id="content-panel" class="panel"><div id="content-tree" class="content-list"><?php foreach ($exports as $i => $ex): ?><div class="content-item" data-filter-row><span><?= name_link_html($pkg, (int)($ex['objectName'] ?? -1), $pkg->exportObjectName((int)($ex['objectName'] ?? -1))) ?></span><span class="content-class"><?= ref_value_html(object_ref_label($pkg, (int)($ex['classIndex'] ?? 0))) ?></span></div><?php endforeach; ?></div><div class="to-top"><a href="#top">To top ↑</a></div></section>
            <section id="externs-panel" class="panel"><?= control_bar('externs-tree') ?><div id="externs-tree" class="tree-box"><?php foreach (($importsByOuter[0] ?? []) as $rootIdx) renderImportNode($pkg, $imports, $importsByOuter, (int)$rootIdx, true, 'externs-'); foreach (($exportsByOuter[0] ?? []) as $rootIdx) renderExportNode($pkg, $exports, $exportsByOuter, (int)$rootIdx, false, 'externs-', true); ?></div><div class="to-top"><a href="#top">To top ↑</a></div></section>
            <section id="tables-panel" class="panel"><div class="subtabs"><button class="subtab active" data-sub="names-sub" onclick="showSub('names-sub')">☰ Names</button><button class="subtab" data-sub="exports-sub" onclick="showSub('exports-sub')">▤ Exports</button><button class="subtab" data-sub="imports-sub" onclick="showSub('imports-sub')">▧ Imports</button><button class="subtab" data-sub="gens-sub" onclick="showSub('gens-sub')">☷ Generations</button></div><div id="names-sub" class="subpanel active"><h2>Names (<?= h($hdr['nameCount'] ?? '') ?>:<?= h($hdr['nameOffset'] ?? '') ?>)</h2><table class="data names-table"><thead><tr><th>Name</th><th>Flags</th><th>Num.</th><th>Raw</th></tr></thead><tbody><?php foreach ($names as $n): $flags=(int)($n['flags']??0); $ni=(int)($n['index']??0); ?><tr id="<?= h(name_ref_target_id($ni)) ?>" data-filter-row><td><?= h($n['name']??'') ?></td><td class="mono raw"><?= h(hx_any($flags)) ?><?= h(flag_names($pkg->decodeRF($flags))) ?></td><td class="mono"><?= h(($n['index']??'').' ('.hx2($ni).')') ?></td><td class="mono raw"><?= h(($n['name']??'').' / '.$flags) ?></td></tr><?php endforeach; ?></tbody></table><div class="to-top"><a href="#top">To top ↑</a></div></div><div id="exports-sub" class="subpanel"><h2>Exports Tree (<?= h($hdr['exportCount'] ?? '') ?>:<?= h($hdr['exportOffset'] ?? '') ?>)</h2><?= control_bar('exports-tree') ?><div id="exports-tree" class="tree-box"><?php foreach ($exports as $i => $_ex) renderExportNode($pkg, $exports, $exportsByOuter, (int)$i, true, 'tables-', false); ?></div><div class="grid-after-tree"><details><summary>Raw Exports Grid</summary><table class="data"><thead><tr><th>Class</th><th>Super</th><th>Package</th><th>Object</th><th>Flags</th><th>Size</th><th>Offset</th><th>Num.</th><th>Raw</th></tr></thead><tbody><?php foreach ($exports as $i=>$ex): $objIdx=(int)($ex['objectName']??-1); ?><tr data-filter-row><td><?= ref_value_html(object_ref_label($pkg,(int)($ex['classIndex']??0))) ?></td><td><?= ref_value_html(object_ref_label($pkg,(int)($ex['superIndex']??0))) ?></td><td><?= ref_value_html(object_ref_label($pkg,(int)($ex['packageIndex']??0))) ?></td><td><?= name_link_html($pkg,$objIdx,$pkg->exportObjectName($objIdx)) ?></td><td class="mono raw"><?= h(hx_any($ex['objectFlags']??0)) ?></td><td><?= h($ex['serialSize']??'') ?></td><td><?= h($ex['serialOffset']??'') ?></td><td><?= h($i.' ('.hx2($i).')') ?></td><td class="mono raw"><?= h(($ex['classIndex']??'').' / '.($ex['superIndex']??'').' / '.($ex['packageIndex']??'').' / ') ?><?= name_index_link($objIdx) ?><?= h(' / '.($ex['objectFlags']??'').' / '.($ex['serialSize']??'').' / '.($ex['serialOffset']??'')) ?></td></tr><?php endforeach; ?></tbody></table></details></div><div class="to-top"><a href="#top">To top ↑</a></div></div><div id="imports-sub" class="subpanel"><h2>Imports Tree (<?= h($hdr['importCount'] ?? '') ?>:<?= h($hdr['importOffset'] ?? '') ?>)</h2><?= control_bar('imports-tree') ?><div id="imports-tree" class="tree-box"><?php foreach ($imports as $i => $_im) renderImportNode($pkg, $imports, $importsByOuter, (int)$i, false, 'tables-'); ?></div><div class="grid-after-tree"><details><summary>Raw Imports Grid</summary><table class="data"><thead><tr><th>Class Package</th><th>Class Name</th><th>Package Name</th><th>Object Name</th><th>Num.</th><th>Raw</th></tr></thead><tbody><?php foreach ($imports as $i=>$im): $cp=(int)($im['classPackage']??-1); $cn=(int)($im['className']??-1); $on=(int)($im['objectName']??-1); ?><tr data-filter-row><td><?= name_link_html($pkg,$cp,$pkg->importClassPackageName($cp)) ?></td><td><?= name_link_html($pkg,$cn,$pkg->importClassName($cn)) ?></td><td><?= ref_value_html(object_ref_label($pkg,(int)($im['outerIndex']??0))) ?></td><td><?= name_link_html($pkg,$on,$pkg->importObjectName($on)) ?></td><td><?= h($i.' ('.hx2($i).')') ?></td><td class="mono raw"><?= name_index_link($cp) ?><?= h(' / ') ?><?= name_index_link($cn) ?><?= h(' / '.($im['outerIndex']??'').' / ') ?><?= name_index_link($on) ?></td></tr><?php endforeach; ?></tbody></table></details></div><div class="to-top"><a href="#top">To top ↑</a></div></div><div id="gens-sub" class="subpanel"><h2>Generations (<?= count($hdr['generations'] ?? []) ?>)</h2><table class="data"><thead><tr><th>ExportCount</th><th>NameCount</th><th>NetObjectCount</th><th>Num.</th><th>Raw</th></tr></thead><tbody><?php foreach (($hdr['generations']??[]) as $i=>$g): ?><tr><td><?= h($g['e']??'') ?></td><td><?= h($g['n']??'') ?></td><td><?= h($g['netObjectCount']??'') ?></td><td><?= h($i) ?></td><td><?= h(($g['e']??'').' / '.($g['n']??'').' / '.($g['netObjectCount']??'')) ?></td></tr><?php endforeach; ?></tbody></table><div class="to-top"><a href="#top">To top ↑</a></div></div></section>
        </div>
    </div>
</body>
</html>
