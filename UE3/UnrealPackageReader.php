...
final class UEFolderBinaryReader
{
    ...
    public function compactIndex(): int{$b=$this->u8();$neg=($b&0x80)!==0;$more=($b&0x40)!==0;$value=$b&0x3F;$shift=6;$count=1;while($more){if(++$count>5)throw new RuntimeException('Invalid compact index length');$b=$this->u8();$more=($b&0x80)!==0;$value|=($b&0x7F)<<$shift;$shift+=7;}return $neg?-$value:$value;}
...
final class UnrealPackageReader
{
    ...
    public function __construct(string $path){$this->path=$path;try{$data=file_get_contents($path);if($data===false)throw new RuntimeException("Failed to read package: $path");$this->bytes=$data;$this->parse();}catch(Throwable $e){$this->issues[]=$this->formatThrowable($e);if(!$this->header)$this->header=$this->blankHeader();}}
    ...
    private function parse(): void{$r=new UEFolderBinaryReader($this->bytes);$tag=$r->u32();if($tag!==0x9E2A83C1)throw new RuntimeException(sprintf('Bad package tag 0x%08X',$tag));$packed=$r->u32();$version=$packed&0xFFFF;$licensee=($packed>>16)&0xFFFF;$ext=strtolower(pathinfo($this->path,PATHINFO_EXTENSION));if(in_array($ext,['ut3','upk','xxx'],true)||$version>=334)$this->parseUE3($r,$tag,$packed,$version,$licensee);else $this->parseUE12($r,$tag,$packed,$version,$licensee);}
