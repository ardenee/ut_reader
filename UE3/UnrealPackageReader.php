...
final class UEFolderBinaryReader
{
    ...
    public function bytes(int $count): string { if ($count < 0 || $this->pos + $count > $this->len) throw new OutOfBoundsException("read overrun need=$count pos={$this->pos} len={$this->len}"); $out = substr($this->buf, $this->pos, $count); $this->pos += $count; return $out; }
    ...
    public function cstring(int $max = 1024): string { $out = ''; for ($i = 0; $i < $max && $this->remaining() > 0; $i++) { $c = $this->bytes(1); if ($c === "\0") break; $out .= $c; } return self::toUtf8($out); }
    ...
    {
        if ($length === 0) return '';
        if ($length > 0) { if ($length > 65536 || $length > $this->remaining()) throw new OutOfBoundsException("bad FString length=$length pos={$this->pos} remaining={$this->remaining()}"); $raw = $this->bytes($length); if ($raw !== '' && substr($raw, -1) === "\0") $raw = substr($raw, 0, -1); return self::toUtf8($raw); }
        $chars = -$length; $bytes = $chars * 2; if ($chars > 32768 || $bytes > $this->remaining()) throw new OutOfBoundsException("bad wide FString length=$length pos={$this->pos} remaining={$this->remaining()}"); $raw = $this->bytes($bytes); if (substr($raw, -2) === "\0\0") $raw = substr($raw, 0, -2); $out = @mb_convert_encoding($raw, 'UTF-8', 'UTF-16LE'); return $out === false ? '' : $out;
    }
...
final class UnrealPackageReader
{
    ...
    {
        if(((int)($this->header['compressionFlags']??0)&self::COMPRESS_ZLIB)!==0){ $out=@gzuncompress($src,$expected); if($out===false)$out=@gzdecode($src); if($out===false)throw new RuntimeException('zlib decompression failed'); return $out; }
        if(((int)($this->header['compressionFlags']??0)&self::COMPRESS_LZO)!==0) throw new RuntimeException('LZO package requires PHP FFI + liblzo2. Enable ffi.enable=true and install liblzo2 on Synology.');
        throw new RuntimeException('unsupported compression flags '.sprintf('0x%08X',(int)($this->header['compressionFlags']??0)));
    }
    ...
    private function readUE3Names(): void
    {
        ...
        $r=$this->tableReader($offset); for($i=0;$i<(int)$this->header['nameCount'];$i++){ if($layout==='flags32-string'){$flags=$r->u32();$name=$r->fstring32();} elseif($layout==='flags64-string'){$flags=$r->u64();$name=$r->fstring32();} else{$name=$r->fstring32();$flags=$r->u64();} $this->names[]=['index'=>$i,'name'=>$name,'flags'=>$flags,'objectFlags'=>$flags]; }
    }
    ...
    private function readUE3StandardExports(): void
    {
        ...
        for($i=0;$i<(int)$this->header['exportCount'];$i++){
            $class=$r->i32(); $super=$r->i32(); $outer=$r->i32(); $objectName=$r->i32(); $objectNameNumber=$r->i32(); $archetype=$v>=220?$r->i32():0; $flagsLo=$r->u32(); $flagsHi=$v>=195?$r->u32():0; $flags=($flagsHi<<32)|$flagsLo; $serialSize=$r->i32(); $serialOffset=($serialSize!==0||$v>=249)?$r->i32():0; $exportFlags=$v>=247?$r->u32():0; $components=[];
            if($v>=220&&$v<543){$componentCount=$r->i32(); if($componentCount<0||$componentCount>65536) throw new RuntimeException("Bad component count $componentCount in export $i at offset ".$r->tell()); for($j=0;$j<$componentCount;$j++)$components[]=['name'=>$r->i32(),'nameNumber'=>$r->i32(),'ref'=>$r->i32()];}
            if($v>=322){for($j=0;$j<16;$j++)$r->i32(); $r->u32(); $r->u32(); $r->u32(); $r->u32();} if($v>=475)$r->i32();
