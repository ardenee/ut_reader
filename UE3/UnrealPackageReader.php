...
final class UnrealPackageReader
{
    ...
    private function setHeaderBase(array $values): void
    {
        $this->header = array_replace($this->blankHeader(), $values);
    }
    ...
    private function parse(): void
    {
        ...
        if (in_array($ext, ['ut3','upk','xxx'], true) || $version >= 334) {
    ...
    private function parseUE12(UEFolderBinaryReader $r, int $tag, int $packed, int $version, int $licensee): void
    {
        ...
        if ($version < 68) {
            ...
            for ($i = 0; $i < $genCount; $i++) {
                $this->header['generations'][] = ['e'=>$r->i32(), 'n'=>$r->i32()];
            }
        }
        $this->readUE12Names();
    ...
    private function parseUE3(UEFolderBinaryReader $r, int $tag, int $packed, int $version, int $licensee): void
    {
        ...
        if ($this->header['compressed']) {
            ...
            for ($i = 0; $i < $chunkCount; $i++) {
                $this->header['chunks'][] = ['uOff'=>$r->u32(), 'uLen'=>$r->u32(), 'cOff'=>$r->u32(), 'cLen'=>$r->u32()];
            }
    ...
    private function readUE12Names(): void { $r = $this->tableReader((int)$this->header['nameOffset']); $v = (int)$this->header['version']; for ($i = 0; $i < (int)$this->header['nameCount']; $i++) { $name = $v < 64 ? $r->cstring() : $r->fstringIndex($v); $flags = $r->u32(); $this->names[] = ['index'=>$i, 'name'=>$name, 'flags'=>$flags, 'objectFlags'=>$flags]; } }
    ...
    private function readUE3Exports(): void
    {
        ...
        if ($v === 512) {
            $this->readUE3CompactUT3Exports();
            return;
        }
        $this->readUE3StandardExports();
    }

    private function readUE3CompactUT3Exports(): void
    {
        $this->header['exportTableLayout'] = 'ut3-v512-compact';
    ...
    private function readUE3StandardExports(): void
    {
        ...
        $v = (int)$this->header['version'];
        for ($i = 0; $i < (int)$this->header['exportCount']; $i++) {
            $class = $r->i32(); $super = $r->i32(); $outer = $r->i32(); $objectName = $r->i32(); $objectNameNumber = $r->i32(); $archetype = $v >= 220 ? $r->i32() : 0;
            $flagsLo = $r->u32(); $flagsHi = $v >= 195 ? $r->u32() : 0; $flags = ($flagsHi << 32) | $flagsLo;
            $serialSize = $r->i32(); $serialOffset = ($serialSize !== 0 || $v >= 249) ? $r->i32() : 0; $exportFlags = $v >= 247 ? $r->u32() : 0; $components = [];
            ...
            if ($v >= 322) { for ($j = 0; $j < 16; $j++) $r->i32(); $r->u32(); $r->u32(); $r->u32(); $r->u32(); }
            if ($v >= 475) $r->i32();
    ...
    private function readUE12PropertyList(int $offset, int $serialSize): array { return []; }

    private function readUE3PropertyList(int $offset, int $serialSize): array
    {
        $r = $this->tableReader($offset); $end = min($r->size(), $offset + $serialSize); $props = [];
        for ($i = 0; $i < 4096 && $r->tell() + 16 <= $end; $i++) {
            $start = $r->tell(); $nameRef = $this->readUE3FName($r); $base = $this->baseNameByIndex($nameRef['index']); $name = $this->nameByIndex($nameRef['index'], $nameRef['number']);
            if ($base === '' || strcasecmp($base, 'None') === 0) break;
            $typeRef = $this->readUE3FName($r); $typeName = $this->baseNameByIndex($typeRef['index']); $typeId = self::PROP_TYPE_NAMES[$typeName] ?? 0; $size = $r->i32(); $idx = $r->i32(); $bool = 0; $struct = '';
            if ($typeId === 10) { $sr = $this->readUE3FName($r); $struct = $this->nameByIndex($sr['index'], $sr['number']); }
            elseif ($typeId === 3) { $bool = $r->i32(); }
            $valOff = $r->tell(); $raw = ''; $value = (bool)$bool;
