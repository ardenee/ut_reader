...
final class UEFolderBinaryReader
{
    ...
    public function peekI32(): int { $s = $this->peek(4); if (strlen($s) !== 4) return 0; $v = (int)unpack('V', $s)[1]; return ($v & 0x80000000) ? $v - 0x100000000 : $v; }
    ...
    {
        $b = $this->u8(); $neg = ($b & 0x80) !== 0; $more = ($b & 0x40) !== 0; $value = $b & 0x3F; $shift = 6; $count = 1;
        while ($more) { if (++$count > 5) throw new RuntimeException('Invalid compact index length'); $b = $this->u8(); $more = ($b & 0x80) !== 0; $value |= ($b & 0x7F) << $shift; $shift += 7; }
...
final class UnrealPackageReader
{
    ...
    {
        $this->path = $path;
        try { $data = file_get_contents($path); if ($data === false) throw new RuntimeException("Failed to read package: $path"); $this->bytes = $data; $this->parse(); }
        catch (Throwable $e) { $this->issues[] = $this->formatThrowable($e); if (!$this->header) $this->header = $this->blankHeader(); }
    }
    ...
    private function parse(): void
    {
        ...
        $packed = $r->u32(); $version = $packed & 0xFFFF; $licensee = ($packed >> 16) & 0xFFFF; $ext = strtolower(pathinfo($this->path, PATHINFO_EXTENSION));
        if (in_array($ext, ['ut3','upk','xxx'], true) || $version >= 334) $this->parseUE3($r, $tag, $packed, $version, $licensee); else $this->parseUE12($r, $tag, $packed, $version, $licensee);
    }
    ...
    {
        if($offset<0||$offset+4>strlen($this->bytes)) return false; $r=$this->tableReader($offset); $len=$r->i32(); if($len>0) return $len<=1024&&$len<=$r->remaining(); if($len<0){ $chars=-$len; return $chars<=1024&&($chars*2)<=$r->remaining(); } return true;
    }
    private function detectUE3NameTableLayout(int $offset): string
    {
        if($this->looksLikeFStringAt($offset)) return 'string-flags';
        $flagBytes=((int)$this->header['version']>=195)?8:4;
        if($this->looksLikeFStringAt($offset+$flagBytes)) return 'flags-string';
        return 'invalid';
    }
    private function readUE12Names(): void
    {
        $r=$this->tableReader((int)$this->header['nameOffset']); $version=(int)$this->header['version']; for($i=0;$i<(int)$this->header['nameCount'];$i++){ $name=$version<64?$r->cstring():$r->fstringIndex($version); $flags=$r->u32(); $this->names[]=['index'=>$i,'name'=>$name,'flags'=>$flags,'objectFlags'=>$flags]; }
    }
    private function readUE12Imports(): void
    {
        $r=$this->tableReader((int)$this->header['importOffset']); $v=(int)$this->header['version']; for($i=0;$i<(int)$this->header['importCount'];$i++) $this->imports[]=$this->makeImport($i,$r->versionIndex($v),0,$r->versionIndex($v),0,$r->i32(),$r->versionIndex($v),0);
    }
    private function readUE12Exports(): void
    {
        $r=$this->tableReader((int)$this->header['exportOffset']); $v=(int)$this->header['version']; for($i=0;$i<(int)$this->header['exportCount'];$i++){ $class=$r->versionIndex($v); $super=$r->versionIndex($v); $outer=$r->i32(); $name=$r->versionIndex($v); $flags=$r->u32(); $size=$r->versionIndex($v); $off=$size>0?$r->versionIndex($v):0; $this->exports[]=$this->makeExport($i,$class,$super,$outer,$name,0,0,$flags,$size,$off); }
    }
    private function readUE3Names(): void
    {
        $offset=(int)$this->header['nameOffset']; $version=(int)$this->header['version']; $layout=$this->detectUE3NameTableLayout($offset); $this->header['nameTableLayout']=$layout; if($layout==='invalid') throw new RuntimeException('Could not detect UE3 name table layout at nameOffset='.$offset.' firstDword='.$this->tableReader($offset)->peekI32());
        $r=$this->tableReader($offset);
        for($i=0;$i<(int)$this->header['nameCount'];$i++){
            if($layout==='flags-string') { $flags=$version>=195?$r->u64():$r->u32(); $name=$r->fstring32(); }
            else { $name=$r->fstring32(); $flags=$version>=195?$r->u64():$r->u32(); }
            $this->names[]=['index'=>$i,'name'=>$name,'flags'=>$flags,'objectFlags'=>$flags];
        }
    }
    private function readUE3Imports(): void
    {
        $r=$this->tableReader((int)$this->header['importOffset']); for($i=0;$i<(int)$this->header['importCount'];$i++) $this->imports[]=$this->makeImport($i,$r->i32(),$r->i32(),$r->i32(),$r->i32(),$r->i32(),$r->i32(),$r->i32());
    }
    private function readUE3Exports(): void
    {
        $r=$this->tableReader((int)$this->header['exportOffset']); $v=(int)$this->header['version']; for($i=0;$i<(int)$this->header['exportCount'];$i++){ $class=$r->i32(); $super=$r->i32(); $outer=$r->i32(); $objectName=$r->i32(); $objectNameNumber=$r->i32(); $archetype=$v>=220?$r->i32():0; $flagsLo=$r->u32(); $flagsHi=$v>=195?$r->u32():0; $flags=($flagsHi<<32)|$flagsLo; $serialSize=$r->i32(); $serialOffset=($serialSize!==0||$v>=249)?$r->i32():0; $components=[]; if($v>=220&&$v<543){ $componentCount=$r->i32(); if($componentCount<0||$componentCount>65536) throw new RuntimeException("Bad component count $componentCount in export $i"); for($j=0;$j<$componentCount;$j++) $components[]=['name'=>$r->i32(),'nameNumber'=>$r->i32(),'ref'=>$r->i32()]; } $exportFlags=$v>=247?$r->u32():0; if($v>=322){ for($j=0;$j<16;$j++) $r->i32(); $r->u32(); $r->u32(); $r->u32(); $r->u32(); } if($v>=475) $r->i32(); $this->exports[]=$this->makeExport($i,$class,$super,$outer,$objectName,$objectNameNumber,$archetype,$flags,$serialSize,$serialOffset,$components,$exportFlags); }
    }
    ...
    public function getCompressionInfo(): array { $chunks=$this->header['chunks']??[]; $tc=0; $tu=0; foreach($chunks as $c){$tc+=(int)($c['cLen']??0);$tu+=(int)($c['uLen']??0);} return ['isCompressed'=>(bool)($this->header['compressed']??false),'flags'=>(int)($this->header['compressionFlags']??0),'chunks'=>$chunks,'totalCompressed'=>$tc,'totalUncompressed'=>$tu,'logicalDecompressed'=>(bool)($this->header['logicalDecompressed']??false),'logicalSize'=>(int)($this->header['logicalSize']??0)]; }
    ...
    public function displayNameFromRef(int $ref): string { if($ref===0)return ''; if($ref>0){$ex=$this->exports[$ref-1]??null; return is_array($ex)?$this->nameByIndex((int)($ex['objectName']??-1),(int)($ex['nameNumber']??0)):'';} $im=$this->imports[-$ref-1]??null; return is_array($im)?$this->nameByIndex((int)($im['objectName']??-1),(int)($im['ObjectName']['number']??0)):''; }
    ...
    public function getExportProperties(int $exportIndex): ?array { if(isset($this->propertyCache[$exportIndex])) return $this->propertyCache[$exportIndex]; $ex=$this->exports[$exportIndex]??null; if(!$ex||(int)$ex['serialSize']<=0||(int)$ex['serialOffset']<=0) return $this->propertyCache[$exportIndex]=[]; try{return $this->propertyCache[$exportIndex]=$this->readPropertyList((int)$ex['serialOffset'],(int)$ex['serialSize']);}catch(Throwable $e){$this->issues[]='Property parse failed for export '.$exportIndex.': '.$e->getMessage();return $this->propertyCache[$exportIndex]=[];} }
    ...
    {
        $v=(int)$this->header['version']; $r=$this->tableReader($offset); $end=min($r->size(),$offset+$serialSize); $props=[]; for($i=0;$i<2048&&$r->tell()<$end;$i++){ $start=$r->tell(); $nameIndex=$r->compactIndex(); $name=$this->nameByIndex($nameIndex); if($name===''||strcasecmp($name,'None')===0) break; $info=$r->u8(); $typeId=$info&0x0F; $sizeCode=($info>>4)&7; $boolFlag=($info&0x80)!==0; $isBool=$typeId===3; $struct=''; if($typeId===10)$struct=$this->nameByIndex($r->compactIndex()); $size=$this->readPropertySize($r,$sizeCode); $idx=(!$isBool&&$boolFlag)?$this->readPropertyArrayIndex($r):0; $valOff=$r->tell(); $raw=''; $val=$isBool?$boolFlag:''; if(!$isBool){$raw=$r->bytes(min($size,max(0,$end-$r->tell())));$val=$this->decodeUE12PropertyValue($typeId,$raw,$v,$struct);} $props[]=$this->makeProp($start,$r->tell(),$name,$typeId,$struct,(!$isBool&&$boolFlag),$isBool?(int)$boolFlag:0,$idx,$sizeCode,$size,$info,$val,$raw,$valOff,'UE1/UE2'); } return $props;
    }
    private function readUE3PropertyList(int $offset,int $serialSize): array
    {
        $v=(int)$this->header['version']; $r=$this->tableReader($offset); $end=min($r->size(),$offset+$serialSize); $props=[]; for($i=0;$i<4096&&$r->tell()<$end;$i++){ $start=$r->tell(); $nameRef=$this->readUE3FName($r); $base=$this->baseNameByIndex($nameRef['index']); $name=$this->nameByIndex($nameRef['index'],$nameRef['number']); if($base===''||strcasecmp($base,'None')===0) break; $typeRef=$this->readUE3FName($r); $typeName=$this->baseNameByIndex($typeRef['index']); $typeId=self::PROP_TYPE_NAMES[$typeName]??0; $size=$r->i32(); $idx=$r->i32(); $struct=''; $bool=0; $enum=''; if($typeId===10){$sr=$this->readUE3FName($r);$struct=$this->nameByIndex($sr['index'],$sr['number']);} elseif($typeId===3){$bool=$v<673?$r->i32():$r->u8();} elseif($typeId===1&&$v>=633){$er=$this->readUE3FName($r);$enum=$this->nameByIndex($er['index'],$er['number']);} $valOff=$r->tell(); $raw=''; $val=(bool)$bool; if($typeId!==3){$raw=$r->bytes(min(max(0,$size),max(0,$end-$r->tell())));$val=$this->decodeUE3PropertyValue($typeId,$raw,$v,$struct);} $props[]=$this->makeProp($start,$r->tell(),$name,$typeId,$struct,false,$bool,$idx,0,$size,0,$val,$raw,$valOff,'UE3',['typeName'=>$typeName,'enumName'=>$enum,'nameIndex'=>$nameRef['index'],'nameNumber'=>$nameRef['number'],'typeIndex'=>$typeRef['index'],'typeNumber'=>$typeRef['number']]); } return $props;
    }
